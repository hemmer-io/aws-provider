...
impl<'a> KafkaconnectService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "custom_plugin" => self.plan_custom_plugin(current_state, desired_input).await,
            "connector" => self.plan_connector(current_state, desired_input).await,
            "worker_configuration" => {
                self.plan_worker_configuration(current_state, desired_input)
                    .await
            }
            "connector_operation" => {
                self.plan_connector_operation(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kafkaconnect", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "custom_plugin" => self.create_custom_plugin(input).await,
            "connector" => self.create_connector(input).await,
            "worker_configuration" => self.create_worker_configuration(input).await,
            "connector_operation" => self.create_connector_operation(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kafkaconnect", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "custom_plugin" => self.read_custom_plugin(id).await,
            "connector" => self.read_connector(id).await,
            "worker_configuration" => self.read_worker_configuration(id).await,
            "connector_operation" => self.read_connector_operation(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kafkaconnect", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "custom_plugin" => self.update_custom_plugin(id, input).await,
            "connector" => self.update_connector(id, input).await,
            "worker_configuration" => self.update_worker_configuration(id, input).await,
            "connector_operation" => self.update_connector_operation(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kafkaconnect", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "custom_plugin" => self.delete_custom_plugin(id).await,
            "connector" => self.delete_connector(id).await,
            "worker_configuration" => self.delete_worker_configuration(id).await,
            "connector_operation" => self.delete_connector_operation(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kafkaconnect", resource_name
            ))),
    ...
    async fn create_custom_plugin(&self, input: ResourceInput) -> Result<ResourceOutput> {
    ...
    async fn read_custom_plugin(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn update_custom_plugin(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let name = input.get_string("name")?;
            let location = input.get_string("location")?;
    ...
    async fn delete_custom_plugin(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_connector(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            let kafka_cluster_encryption_in_transit =
                input.get_string("kafka_cluster_encryption_in_transit")?;
            ...
            let kafka_cluster_client_authentication =
                input.get_string("kafka_cluster_client_authentication")?;
            let tags = input.get_optional_string("tags")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    kafka_cluster_encryption_in_transit.unwrap_or_default(),
                )
                .with_field(
                    "worker_configuration",
                    worker_configuration.unwrap_or_default(),
                )
                .with_field("plugins", plugins.unwrap_or_default())
                .with_field("kafka_cluster", kafka_cluster.unwrap_or_default())
                .with_field(
                    "kafka_cluster_client_authentication",
                    kafka_cluster_client_authentication.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default())
                .with_field(
                    "connector_description",
                    connector_description.unwrap_or_default(),
                )
                .with_field("connector_name", connector_name.unwrap_or_default())
                .with_field(
                    "connector_configuration",
                    connector_configuration.unwrap_or_default(),
                )
                .with_field("capacity", capacity.unwrap_or_default())
                .with_field(
                    "kafka_connect_version",
                    kafka_connect_version.unwrap_or_default(),
                )
    ...
    async fn create_worker_configuration(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let description = input.get_optional_string("description")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    properties_file_content.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default())
                .with_field("name", name.unwrap_or_default()))
        })
    ...
    async fn read_worker_configuration(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kafkaconnect_client
    ...
    async fn delete_worker_configuration(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kafkaconnect_client
    ...
    async fn create_connector_operation(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_connector_operation(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kafkaconnect_client
    ...
    async fn delete_connector_operation(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kafkaconnect_client
