...
impl<'a> Compute_optimizerService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "rds_database_recommendation_projected_metrics" => {
                self.plan_rds_database_recommendation_projected_metrics(
                    current_state,
                    desired_input,
                )
                .await
            ...
            "lambda_function_recommendations" => {
                self.plan_lambda_function_recommendations(current_state, desired_input)
                    .await
            }
            "idle_recommendations" => {
                self.plan_idle_recommendations(current_state, desired_input)
                    .await
            }
            "auto_scaling_group_recommendations" => {
                self.plan_auto_scaling_group_recommendations(current_state, desired_input)
                    .await
            }
            "recommendation_export_jobs" => {
                self.plan_recommendation_export_jobs(current_state, desired_input)
                    .await
            }
            "rds_database_recommendations" => {
                self.plan_rds_database_recommendations(current_state, desired_input)
                    .await
            }
            "ec2_recommendation_projected_metrics" => {
                self.plan_ec2_recommendation_projected_metrics(current_state, desired_input)
                    .await
            }
            "effective_recommendation_preferences" => {
                self.plan_effective_recommendation_preferences(current_state, desired_input)
                    .await
            }
            "ecs_service_recommendation_projected_metrics" => {
                self.plan_ecs_service_recommendation_projected_metrics(current_state, desired_input)
                    .await
            }
            "ec2_instance_recommendations" => {
                self.plan_ec2_instance_recommendations(current_state, desired_input)
                    .await
            }
            "license_recommendations" => {
                self.plan_license_recommendations(current_state, desired_input)
                    .await
            }
            "ecs_service_recommendations" => {
                self.plan_ecs_service_recommendations(current_state, desired_input)
                    .await
            }
            "enrollment_status" => {
                self.plan_enrollment_status(current_state, desired_input)
                    .await
            }
            "recommendation_summaries" => {
                self.plan_recommendation_summaries(current_state, desired_input)
                    .await
            }
            "enrollment_statuses_for_organization" => {
                self.plan_enrollment_statuses_for_organization(current_state, desired_input)
                    .await
            }
            "ebs_volume_recommendations" => {
                self.plan_ebs_volume_recommendations(current_state, desired_input)
                    .await
            }
            "recommendation_preferences" => {
                self.plan_recommendation_preferences(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "compute_optimizer", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "rds_database_recommendation_projected_metrics" => {
                self.create_rds_database_recommendation_projected_metrics(input)
                    .await
            }
            "lambda_function_recommendations" => {
                self.create_lambda_function_recommendations(input).await
            }
            "idle_recommendations" => self.create_idle_recommendations(input).await,
            "auto_scaling_group_recommendations" => {
                self.create_auto_scaling_group_recommendations(input).await
            }
            "recommendation_export_jobs" => self.create_recommendation_export_jobs(input).await,
            "rds_database_recommendations" => self.create_rds_database_recommendations(input).await,
            "ec2_recommendation_projected_metrics" => {
                self.create_ec2_recommendation_projected_metrics(input)
                    .await
            }
            "effective_recommendation_preferences" => {
                self.create_effective_recommendation_preferences(input)
                    .await
            }
            "ecs_service_recommendation_projected_metrics" => {
                self.create_ecs_service_recommendation_projected_metrics(input)
                    .await
            }
            "ec2_instance_recommendations" => self.create_ec2_instance_recommendations(input).await,
            "license_recommendations" => self.create_license_recommendations(input).await,
            "ecs_service_recommendations" => self.create_ecs_service_recommendations(input).await,
            "enrollment_status" => self.create_enrollment_status(input).await,
            "recommendation_summaries" => self.create_recommendation_summaries(input).await,
            "enrollment_statuses_for_organization" => {
                self.create_enrollment_statuses_for_organization(input)
                    .await
            }
            "ebs_volume_recommendations" => self.create_ebs_volume_recommendations(input).await,
            "recommendation_preferences" => self.create_recommendation_preferences(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "compute_optimizer", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "rds_database_recommendation_projected_metrics" => {
                self.read_rds_database_recommendation_projected_metrics(id)
                    .await
            }
            "lambda_function_recommendations" => {
                self.read_lambda_function_recommendations(id).await
            }
            "idle_recommendations" => self.read_idle_recommendations(id).await,
            "auto_scaling_group_recommendations" => {
                self.read_auto_scaling_group_recommendations(id).await
            }
            "recommendation_export_jobs" => self.read_recommendation_export_jobs(id).await,
            "rds_database_recommendations" => self.read_rds_database_recommendations(id).await,
            "ec2_recommendation_projected_metrics" => {
            ...
            "ecs_service_recommendation_projected_metrics" => {
                self.read_ecs_service_recommendation_projected_metrics(id)
                    .await
            }
            "ec2_instance_recommendations" => self.read_ec2_instance_recommendations(id).await,
            "license_recommendations" => self.read_license_recommendations(id).await,
            "ecs_service_recommendations" => self.read_ecs_service_recommendations(id).await,
            "enrollment_status" => self.read_enrollment_status(id).await,
            "recommendation_summaries" => self.read_recommendation_summaries(id).await,
            "enrollment_statuses_for_organization" => {
                self.read_enrollment_statuses_for_organization(id).await
            }
            "ebs_volume_recommendations" => self.read_ebs_volume_recommendations(id).await,
            "recommendation_preferences" => self.read_recommendation_preferences(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "compute_optimizer", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "rds_database_recommendation_projected_metrics" => {
                self.update_rds_database_recommendation_projected_metrics(id, input)
                    .await
            }
            "lambda_function_recommendations" => {
                self.update_lambda_function_recommendations(id, input).await
            }
            "idle_recommendations" => self.update_idle_recommendations(id, input).await,
            "auto_scaling_group_recommendations" => {
                self.update_auto_scaling_group_recommendations(id, input)
                    .await
            }
            "recommendation_export_jobs" => self.update_recommendation_export_jobs(id, input).await,
            "rds_database_recommendations" => {
                self.update_rds_database_recommendations(id, input).await
            }
            "ec2_recommendation_projected_metrics" => {
                self.update_ec2_recommendation_projected_metrics(id, input)
                    .await
            }
            "effective_recommendation_preferences" => {
                self.update_effective_recommendation_preferences(id, input)
                    .await
            }
            "ecs_service_recommendation_projected_metrics" => {
                self.update_ecs_service_recommendation_projected_metrics(id, input)
                    .await
            }
            "ec2_instance_recommendations" => {
                self.update_ec2_instance_recommendations(id, input).await
            }
            "license_recommendations" => self.update_license_recommendations(id, input).await,
            "ecs_service_recommendations" => {
                self.update_ecs_service_recommendations(id, input).await
            }
            "enrollment_status" => self.update_enrollment_status(id, input).await,
            "recommendation_summaries" => self.update_recommendation_summaries(id, input).await,
            "enrollment_statuses_for_organization" => {
                self.update_enrollment_statuses_for_organization(id, input)
                    .await
            }
            "ebs_volume_recommendations" => self.update_ebs_volume_recommendations(id, input).await,
            "recommendation_preferences" => self.update_recommendation_preferences(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "compute_optimizer", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "rds_database_recommendation_projected_metrics" => {
                self.delete_rds_database_recommendation_projected_metrics(id)
                    .await
            }
            "lambda_function_recommendations" => {
                self.delete_lambda_function_recommendations(id).await
            }
            "idle_recommendations" => self.delete_idle_recommendations(id).await,
            "auto_scaling_group_recommendations" => {
                self.delete_auto_scaling_group_recommendations(id).await
            }
            "recommendation_export_jobs" => self.delete_recommendation_export_jobs(id).await,
            "rds_database_recommendations" => self.delete_rds_database_recommendations(id).await,
            "ec2_recommendation_projected_metrics" => {
            ...
            "ecs_service_recommendation_projected_metrics" => {
                self.delete_ecs_service_recommendation_projected_metrics(id)
                    .await
            }
            "ec2_instance_recommendations" => self.delete_ec2_instance_recommendations(id).await,
            "license_recommendations" => self.delete_license_recommendations(id).await,
            "ecs_service_recommendations" => self.delete_ecs_service_recommendations(id).await,
            "enrollment_status" => self.delete_enrollment_status(id).await,
            "recommendation_summaries" => self.delete_recommendation_summaries(id).await,
            "enrollment_statuses_for_organization" => {
                self.delete_enrollment_statuses_for_organization(id).await
            }
            "ebs_volume_recommendations" => self.delete_ebs_volume_recommendations(id).await,
            "recommendation_preferences" => self.delete_recommendation_preferences(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "compute_optimizer", resource_name
            ))),
    ...
    async fn delete_rds_database_recommendation_projected_metrics(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn read_lambda_function_recommendations(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_lambda_function_recommendations(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn create_idle_recommendations(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_idle_recommendations(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_idle_recommendations(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn read_auto_scaling_group_recommendations(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_auto_scaling_group_recommendations(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn read_recommendation_export_jobs(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_recommendation_export_jobs(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn read_rds_database_recommendations(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_rds_database_recommendations(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn read_ec2_recommendation_projected_metrics(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_ec2_recommendation_projected_metrics(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn read_effective_recommendation_preferences(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_effective_recommendation_preferences(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn delete_ecs_service_recommendation_projected_metrics(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn read_ec2_instance_recommendations(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_ec2_instance_recommendations(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn create_license_recommendations(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_license_recommendations(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_license_recommendations(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn read_ecs_service_recommendations(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_ecs_service_recommendations(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn create_enrollment_status(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    include_member_accounts.unwrap_or_default(),
                )
    ...
    async fn read_enrollment_status(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_enrollment_status(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn read_recommendation_summaries(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_recommendation_summaries(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn read_enrollment_statuses_for_organization(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_enrollment_statuses_for_organization(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn read_ebs_volume_recommendations(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_ebs_volume_recommendations(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
    ...
    async fn create_recommendation_preferences(
        ...
        self.provider.runtime().block_on(async {
            ...
            let external_metrics_preference =
                input.get_optional_string("external_metrics_preference")?;
            ...
            let enhanced_infrastructure_metrics =
                input.get_optional_string("enhanced_infrastructure_metrics")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    utilization_preferences.unwrap_or_default(),
                )
                .with_field(
                    "external_metrics_preference",
                    external_metrics_preference.unwrap_or_default(),
                )
                .with_field(
                    "preferred_resources",
                    preferred_resources.unwrap_or_default(),
                )
                .with_field("scope", scope.unwrap_or_default())
                .with_field(
                    "savings_estimation_mode",
                    savings_estimation_mode.unwrap_or_default(),
                )
                .with_field(
                    "inferred_workload_types",
                    inferred_workload_types.unwrap_or_default(),
                )
                .with_field(
                    "enhanced_infrastructure_metrics",
                    enhanced_infrastructure_metrics.unwrap_or_default(),
                )
    ...
    async fn read_recommendation_preferences(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.compute_optimizer_client
    ...
    async fn delete_recommendation_preferences(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.compute_optimizer_client
