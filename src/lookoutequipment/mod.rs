...
impl<'a> LookoutequipmentService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "data_ingestion_job" => {
                self.plan_data_ingestion_job(current_state, desired_input)
                    .await
            }
            "inference_scheduler" => {
                self.plan_inference_scheduler(current_state, desired_input)
                    .await
            }
            "model_version" => self.plan_model_version(current_state, desired_input).await,
            "active_model_version" => {
                self.plan_active_model_version(current_state, desired_input)
                    .await
            }
            ...
            "retraining_scheduler" => {
                self.plan_retraining_scheduler(current_state, desired_input)
                    .await
            }
            "resource_policy" => {
            ...
            "label_group" => self.plan_label_group(current_state, desired_input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "lookoutequipment", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "data_ingestion_job" => self.create_data_ingestion_job(input).await,
            "inference_scheduler" => self.create_inference_scheduler(input).await,
            "model_version" => self.create_model_version(input).await,
            "active_model_version" => self.create_active_model_version(input).await,
            "label" => self.create_label(input).await,
            "model" => self.create_model(input).await,
            "dataset" => self.create_dataset(input).await,
            "retraining_scheduler" => self.create_retraining_scheduler(input).await,
            "resource_policy" => self.create_resource_policy(input).await,
            "label_group" => self.create_label_group(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "lookoutequipment", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "data_ingestion_job" => self.read_data_ingestion_job(id).await,
            "inference_scheduler" => self.read_inference_scheduler(id).await,
            "model_version" => self.read_model_version(id).await,
            "active_model_version" => self.read_active_model_version(id).await,
            "label" => self.read_label(id).await,
            "model" => self.read_model(id).await,
            "dataset" => self.read_dataset(id).await,
            "retraining_scheduler" => self.read_retraining_scheduler(id).await,
            "resource_policy" => self.read_resource_policy(id).await,
            "label_group" => self.read_label_group(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "lookoutequipment", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "data_ingestion_job" => self.update_data_ingestion_job(id, input).await,
            "inference_scheduler" => self.update_inference_scheduler(id, input).await,
            "model_version" => self.update_model_version(id, input).await,
            "active_model_version" => self.update_active_model_version(id, input).await,
            "label" => self.update_label(id, input).await,
            "model" => self.update_model(id, input).await,
            "dataset" => self.update_dataset(id, input).await,
            "retraining_scheduler" => self.update_retraining_scheduler(id, input).await,
            "resource_policy" => self.update_resource_policy(id, input).await,
            "label_group" => self.update_label_group(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "lookoutequipment", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "data_ingestion_job" => self.delete_data_ingestion_job(id).await,
            "inference_scheduler" => self.delete_inference_scheduler(id).await,
            "model_version" => self.delete_model_version(id).await,
            "active_model_version" => self.delete_active_model_version(id).await,
            "label" => self.delete_label(id).await,
            "model" => self.delete_model(id).await,
            "dataset" => self.delete_dataset(id).await,
            "retraining_scheduler" => self.delete_retraining_scheduler(id).await,
            "resource_policy" => self.delete_resource_policy(id).await,
            "label_group" => self.delete_label_group(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "lookoutequipment", resource_name
            ))),
    ...
    async fn create_data_ingestion_job(&self, input: ResourceInput) -> Result<ResourceOutput> {
    ...
    async fn read_data_ingestion_job(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_data_ingestion_job(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_inference_scheduler(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            let data_delay_offset_in_minutes =
                input.get_optional_string("data_delay_offset_in_minutes")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    inference_scheduler_name.unwrap_or_default(),
                )
                .with_field(
                    "data_input_configuration",
                    data_input_configuration.unwrap_or_default(),
                )
                .with_field(
                    "data_output_configuration",
                    data_output_configuration.unwrap_or_default(),
                )
                .with_field(
                    "server_side_kms_key_id",
                    server_side_kms_key_id.unwrap_or_default(),
                )
                .with_field(
                    "data_delay_offset_in_minutes",
                    data_delay_offset_in_minutes.unwrap_or_default(),
                )
                .with_field(
                    "data_upload_frequency",
                    data_upload_frequency.unwrap_or_default(),
                )
    ...
    async fn read_inference_scheduler(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lookoutequipment_client
    ...
    async fn delete_inference_scheduler(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lookoutequipment_client
    ...
    async fn create_active_model_version(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_active_model_version(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lookoutequipment_client
    ...
    async fn delete_active_model_version(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lookoutequipment_client
    ...
    async fn create_model(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            let evaluation_data_start_time =
                input.get_optional_string("evaluation_data_start_time")?;
            let tags = input.get_optional_string("tags")?;
            let training_data_end_time = input.get_optional_string("training_data_end_time")?;
            let labels_input_configuration =
                input.get_optional_string("labels_input_configuration")?;
            let training_data_start_time = input.get_optional_string("training_data_start_time")?;
            let client_token = input.get_string("client_token")?;
            let model_diagnostics_output_configuration =
                input.get_optional_string("model_diagnostics_output_configuration")?;
            let model_name = input.get_string("model_name")?;
            let dataset_schema = input.get_optional_string("dataset_schema")?;
            let server_side_kms_key_id = input.get_optional_string("server_side_kms_key_id")?;
            let data_pre_processing_configuration =
                input.get_optional_string("data_pre_processing_configuration")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    evaluation_data_start_time.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default())
                .with_field(
                    "training_data_end_time",
                    training_data_end_time.unwrap_or_default(),
                )
                .with_field(
                    "labels_input_configuration",
                    labels_input_configuration.unwrap_or_default(),
                )
                .with_field(
                    "training_data_start_time",
                    training_data_start_time.unwrap_or_default(),
                )
                .with_field("client_token", client_token.unwrap_or_default())
                .with_field(
                    "model_diagnostics_output_configuration",
                    model_diagnostics_output_configuration.unwrap_or_default(),
                )
                .with_field("model_name", model_name.unwrap_or_default())
                ...
                .with_field(
                    ...
                    data_pre_processing_configuration.unwrap_or_default(),
                )
                .with_field("dataset_name", dataset_name.unwrap_or_default())
                .with_field(
                    "evaluation_data_end_time",
                    evaluation_data_end_time.unwrap_or_default(),
                )
                .with_field("role_arn", role_arn.unwrap_or_default()))
        })
    }
    ...
    async fn create_retraining_scheduler(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let model_name = input.get_string("model_name")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    retraining_start_date.unwrap_or_default(),
                )
                .with_field("client_token", client_token.unwrap_or_default())
                .with_field(
                    "retraining_frequency",
                    retraining_frequency.unwrap_or_default(),
                )
    ...
    async fn read_retraining_scheduler(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lookoutequipment_client
    ...
    async fn delete_retraining_scheduler(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lookoutequipment_client
    ...
    async fn create_label_group(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_label_group(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lookoutequipment_client
    ...
    async fn update_label_group(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let fault_codes = input.get_optional_string("fault_codes")?;
            let client_token = input.get_string("client_token")?;
    ...
    async fn delete_label_group(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lookoutequipment_client
