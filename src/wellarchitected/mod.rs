...
impl<'a> WellarchitectedService<'a> {
    ...
    async fn create_share_invitation(
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                .with_id("placeholder-id")
                .with_field("share_invitation_action", share_invitation_action.unwrap_or_default())
                .with_field("share_invitation_id", share_invitation_id.unwrap_or_default())
            )
        })
    }

    /// Read a share_invitation resource
    async fn read_share_invitation(
        &self,
        id: &str,
    ) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_share_invitation(
        &self,
        id: &str,
    ) -> Result<()> {
        self.provider.runtime().block_on(async {
