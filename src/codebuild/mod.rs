...
impl<'a> CodebuildService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "report_group_trend" => {
                self.plan_report_group_trend(current_state, desired_input)
                    .await
            }
            "fleet" => self.plan_fleet(current_state, desired_input).await,
            "code_coverages" => self.plan_code_coverages(current_state, desired_input).await,
            "resource_policy" => {
                self.plan_resource_policy(current_state, desired_input)
                    .await
            }
            "report_group" => self.plan_report_group(current_state, desired_input).await,
            "report" => self.plan_report(current_state, desired_input).await,
            "project_visibility" => {
                self.plan_project_visibility(current_state, desired_input)
                    .await
            }
            "project" => self.plan_project(current_state, desired_input).await,
            "webhook" => self.plan_webhook(current_state, desired_input).await,
            "build_batch" => self.plan_build_batch(current_state, desired_input).await,
            "source_credentials" => {
                self.plan_source_credentials(current_state, desired_input)
                    .await
            }
            "test_cases" => self.plan_test_cases(current_state, desired_input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "codebuild", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "report_group_trend" => self.create_report_group_trend(input).await,
            "fleet" => self.create_fleet(input).await,
            "code_coverages" => self.create_code_coverages(input).await,
            "resource_policy" => self.create_resource_policy(input).await,
            "report_group" => self.create_report_group(input).await,
            "report" => self.create_report(input).await,
            "project_visibility" => self.create_project_visibility(input).await,
            "project" => self.create_project(input).await,
            "webhook" => self.create_webhook(input).await,
            "build_batch" => self.create_build_batch(input).await,
            "source_credentials" => self.create_source_credentials(input).await,
            "test_cases" => self.create_test_cases(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "codebuild", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "report_group_trend" => self.read_report_group_trend(id).await,
            "fleet" => self.read_fleet(id).await,
            "code_coverages" => self.read_code_coverages(id).await,
            "resource_policy" => self.read_resource_policy(id).await,
            "report_group" => self.read_report_group(id).await,
            "report" => self.read_report(id).await,
            "project_visibility" => self.read_project_visibility(id).await,
            "project" => self.read_project(id).await,
            "webhook" => self.read_webhook(id).await,
            "build_batch" => self.read_build_batch(id).await,
            "source_credentials" => self.read_source_credentials(id).await,
            "test_cases" => self.read_test_cases(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "codebuild", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "report_group_trend" => self.update_report_group_trend(id, input).await,
            "fleet" => self.update_fleet(id, input).await,
            "code_coverages" => self.update_code_coverages(id, input).await,
            "resource_policy" => self.update_resource_policy(id, input).await,
            "report_group" => self.update_report_group(id, input).await,
            "report" => self.update_report(id, input).await,
            "project_visibility" => self.update_project_visibility(id, input).await,
            "project" => self.update_project(id, input).await,
            "webhook" => self.update_webhook(id, input).await,
            "build_batch" => self.update_build_batch(id, input).await,
            "source_credentials" => self.update_source_credentials(id, input).await,
            "test_cases" => self.update_test_cases(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "codebuild", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "report_group_trend" => self.delete_report_group_trend(id).await,
            "fleet" => self.delete_fleet(id).await,
            "code_coverages" => self.delete_code_coverages(id).await,
            "resource_policy" => self.delete_resource_policy(id).await,
            "report_group" => self.delete_report_group(id).await,
            "report" => self.delete_report(id).await,
            "project_visibility" => self.delete_project_visibility(id).await,
            "project" => self.delete_project(id).await,
            "webhook" => self.delete_webhook(id).await,
            "build_batch" => self.delete_build_batch(id).await,
            "source_credentials" => self.delete_source_credentials(id).await,
            "test_cases" => self.delete_test_cases(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "codebuild", resource_name
            ))),
    ...
    async fn create_report_group_trend(&self, input: ResourceInput) -> Result<ResourceOutput> {
    ...
    async fn read_report_group_trend(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_report_group_trend(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_code_coverages(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_code_coverages(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.codebuild_client
    ...
    async fn delete_code_coverages(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.codebuild_client
    ...
    async fn create_report_group(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let name = input.get_string("name")?;
    ...
    async fn read_report_group(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.codebuild_client
    ...
    async fn update_report_group(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let name = input.get_string("name")?;
            let tags = input.get_optional_string("tags")?;
    ...
    async fn delete_report_group(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.codebuild_client
    ...
    async fn create_project_visibility(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    resource_access_role.unwrap_or_default(),
                ))
        })
    }

    /// Read a project_visibility resource
    async fn read_project_visibility(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.codebuild_client
    ...
    async fn delete_project_visibility(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.codebuild_client
    ...
    async fn create_project(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            // Extract input fields
            let vpc_config = input.get_optional_string("vpc_config")?;
            let queued_timeout_in_minutes =
                input.get_optional_string("queued_timeout_in_minutes")?;
            ...
            let tags = input.get_optional_string("tags")?;
            let secondary_source_versions =
                input.get_optional_string("secondary_source_versions")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    queued_timeout_in_minutes.unwrap_or_default(),
                )
                .with_field("build_batch_config", build_batch_config.unwrap_or_default())
                .with_field(
                    "secondary_artifacts",
                    secondary_artifacts.unwrap_or_default(),
                )
                .with_field(
                    "concurrent_build_limit",
                    concurrent_build_limit.unwrap_or_default(),
                )
                ...
                .with_field(
                    ...
                    secondary_source_versions.unwrap_or_default(),
                )
                ...
                .with_field(
                    ...
                    file_system_locations.unwrap_or_default(),
                )
    ...
    async fn create_webhook(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            let pull_request_build_policy =
                input.get_optional_string("pull_request_build_policy")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    pull_request_build_policy.unwrap_or_default(),
                )
                .with_field(
                    "scope_configuration",
                    scope_configuration.unwrap_or_default(),
                )
    ...
    async fn create_build_batch(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_build_batch(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.codebuild_client
    ...
    async fn update_build_batch(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_build_batch(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.codebuild_client
    ...
    async fn create_source_credentials(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_source_credentials(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.codebuild_client
    ...
    async fn delete_source_credentials(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.codebuild_client
    ...
    async fn create_test_cases(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_test_cases(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.codebuild_client
    ...
    async fn update_test_cases(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_test_cases(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.codebuild_client
