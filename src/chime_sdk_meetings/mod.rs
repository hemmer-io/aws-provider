...
impl<'a> Chime_sdk_meetingsService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "attendee" => self.plan_attendee(current_state, desired_input).await,
            "attendee_capabilities" => {
                self.plan_attendee_capabilities(current_state, desired_input)
                    .await
            }
            "meeting" => self.plan_meeting(current_state, desired_input).await,
            "meeting_with_attendees" => {
                self.plan_meeting_with_attendees(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "chime_sdk_meetings", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "attendee" => self.create_attendee(input).await,
            "attendee_capabilities" => self.create_attendee_capabilities(input).await,
            "meeting" => self.create_meeting(input).await,
            "meeting_with_attendees" => self.create_meeting_with_attendees(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "chime_sdk_meetings", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "attendee" => self.read_attendee(id).await,
            "attendee_capabilities" => self.read_attendee_capabilities(id).await,
            "meeting" => self.read_meeting(id).await,
            "meeting_with_attendees" => self.read_meeting_with_attendees(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "chime_sdk_meetings", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "attendee" => self.update_attendee(id, input).await,
            "attendee_capabilities" => self.update_attendee_capabilities(id, input).await,
            "meeting" => self.update_meeting(id, input).await,
            "meeting_with_attendees" => self.update_meeting_with_attendees(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "chime_sdk_meetings", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "attendee" => self.delete_attendee(id).await,
            "attendee_capabilities" => self.delete_attendee_capabilities(id).await,
            "meeting" => self.delete_meeting(id).await,
            "meeting_with_attendees" => self.delete_meeting_with_attendees(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "chime_sdk_meetings", resource_name
            ))),
    ...
    async fn create_attendee(&self, input: ResourceInput) -> Result<ResourceOutput> {
    ...
    async fn read_attendee(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn update_attendee(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let meeting_id = input.get_string("meeting_id")?;
            let capabilities = input.get_optional_string("capabilities")?;
    ...
    async fn delete_attendee(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_attendee_capabilities(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_attendee_capabilities(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.chime_sdk_meetings_client
    ...
    async fn delete_attendee_capabilities(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.chime_sdk_meetings_client
    ...
    async fn create_meeting(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            let notifications_configuration =
                input.get_optional_string("notifications_configuration")?;
            let primary_meeting_id = input.get_optional_string("primary_meeting_id")?;
            let media_placement_network_type =
                input.get_optional_string("media_placement_network_type")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "external_meeting_id",
                    external_meeting_id.unwrap_or_default(),
                )
                .with_field(
                    "notifications_configuration",
                    notifications_configuration.unwrap_or_default(),
                )
                .with_field("primary_meeting_id", primary_meeting_id.unwrap_or_default())
                .with_field(
                    "media_placement_network_type",
                    media_placement_network_type.unwrap_or_default(),
                )
                .with_field(
    ...
    async fn read_meeting(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.chime_sdk_meetings_client
    ...
    async fn update_meeting(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let media_region = input.get_string("media_region")?;
            let tenant_ids = input.get_optional_string("tenant_ids")?;
    ...
    async fn create_meeting_with_attendees(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let tenant_ids = input.get_optional_string("tenant_ids")?;
    ...
    async fn read_meeting_with_attendees(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.chime_sdk_meetings_client
    ...
    async fn delete_meeting_with_attendees(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.chime_sdk_meetings_client
