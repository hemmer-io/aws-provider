...
impl<'a> KeyspacesService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "keyspace" => self.plan_keyspace(current_state, desired_input).await,
            ...
            "table_auto_scaling_settings" => {
                self.plan_table_auto_scaling_settings(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "keyspaces", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "keyspace" => self.create_keyspace(input).await,
            "table" => self.create_table(input).await,
            "type_" => self.create_type_(input).await,
            "table_auto_scaling_settings" => self.create_table_auto_scaling_settings(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "keyspaces", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "keyspace" => self.read_keyspace(id).await,
            "table" => self.read_table(id).await,
            "type_" => self.read_type_(id).await,
            "table_auto_scaling_settings" => self.read_table_auto_scaling_settings(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "keyspaces", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "keyspace" => self.update_keyspace(id, input).await,
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "keyspace" => self.delete_keyspace(id).await,
            "table" => self.delete_table(id).await,
            "type_" => self.delete_type_(id).await,
            "table_auto_scaling_settings" => self.delete_table_auto_scaling_settings(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "keyspaces", resource_name
            ))),
    ...
    async fn create_keyspace(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let keyspace_name = input.get_string("keyspace_name")?;
            let tags = input.get_optional_string("tags")?;
            let replication_specification =
                input.get_optional_string("replication_specification")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    replication_specification.unwrap_or_default(),
                ))
        })
    }

    /// Read a keyspace resource
    async fn read_keyspace(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn update_keyspace(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let keyspace_name = input.get_string("keyspace_name")?;
            let tags = input.get_optional_string("tags")?;
    ...
    async fn delete_keyspace(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_table(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            let keyspace_name = input.get_string("keyspace_name")?;
            let auto_scaling_specification =
                input.get_optional_string("auto_scaling_specification")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    point_in_time_recovery.unwrap_or_default(),
                )
                .with_field(
                    "capacity_specification",
                    capacity_specification.unwrap_or_default(),
                )
                ...
                .with_field(
                    ...
                    encryption_specification.unwrap_or_default(),
                )
                .with_field(
                    "replica_specifications",
                    replica_specifications.unwrap_or_default(),
                )
                .with_field(
                    "default_time_to_live",
                    default_time_to_live.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default())
                .with_field(
                    "client_side_timestamps",
                    client_side_timestamps.unwrap_or_default(),
                )
                .with_field("keyspace_name", keyspace_name.unwrap_or_default())
                .with_field(
                    "auto_scaling_specification",
                    auto_scaling_specification.unwrap_or_default(),
                ))
        })
    }
    ...
    async fn read_table_auto_scaling_settings(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.keyspaces_client
    ...
    async fn delete_table_auto_scaling_settings(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.keyspaces_client
