...
impl<'a> KendraService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "faq" => self.plan_faq(current_state, desired_input).await,
            "access_control_configuration" => {
                self.plan_access_control_configuration(current_state, desired_input)
                    .await
            }
            "data_source" => self.plan_data_source(current_state, desired_input).await,
            "query_suggestions_config" => {
                self.plan_query_suggestions_config(current_state, desired_input)
                    .await
            }
            "index" => self.plan_index(current_state, desired_input).await,
            "principal_mapping" => {
                self.plan_principal_mapping(current_state, desired_input)
                    .await
            }
            "featured_results_set" => {
                self.plan_featured_results_set(current_state, desired_input)
                    .await
            }
            "thesaurus" => self.plan_thesaurus(current_state, desired_input).await,
            "experience" => self.plan_experience(current_state, desired_input).await,
            "snapshots" => self.plan_snapshots(current_state, desired_input).await,
            "query_suggestions_block_list" => {
                self.plan_query_suggestions_block_list(current_state, desired_input)
                    .await
            }
            "query_suggestions" => {
                self.plan_query_suggestions(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kendra", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "faq" => self.create_faq(input).await,
            "access_control_configuration" => self.create_access_control_configuration(input).await,
            "data_source" => self.create_data_source(input).await,
            "query_suggestions_config" => self.create_query_suggestions_config(input).await,
            "index" => self.create_index(input).await,
            "principal_mapping" => self.create_principal_mapping(input).await,
            "featured_results_set" => self.create_featured_results_set(input).await,
            "thesaurus" => self.create_thesaurus(input).await,
            "experience" => self.create_experience(input).await,
            "snapshots" => self.create_snapshots(input).await,
            "query_suggestions_block_list" => self.create_query_suggestions_block_list(input).await,
            "query_suggestions" => self.create_query_suggestions(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kendra", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "faq" => self.read_faq(id).await,
            "access_control_configuration" => self.read_access_control_configuration(id).await,
            "data_source" => self.read_data_source(id).await,
            "query_suggestions_config" => self.read_query_suggestions_config(id).await,
            "index" => self.read_index(id).await,
            "principal_mapping" => self.read_principal_mapping(id).await,
            "featured_results_set" => self.read_featured_results_set(id).await,
            "thesaurus" => self.read_thesaurus(id).await,
            "experience" => self.read_experience(id).await,
            "snapshots" => self.read_snapshots(id).await,
            "query_suggestions_block_list" => self.read_query_suggestions_block_list(id).await,
            "query_suggestions" => self.read_query_suggestions(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kendra", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "faq" => self.update_faq(id, input).await,
            "access_control_configuration" => {
                self.update_access_control_configuration(id, input).await
            }
            "data_source" => self.update_data_source(id, input).await,
            "query_suggestions_config" => self.update_query_suggestions_config(id, input).await,
            "index" => self.update_index(id, input).await,
            "principal_mapping" => self.update_principal_mapping(id, input).await,
            "featured_results_set" => self.update_featured_results_set(id, input).await,
            "thesaurus" => self.update_thesaurus(id, input).await,
            "experience" => self.update_experience(id, input).await,
            "snapshots" => self.update_snapshots(id, input).await,
            "query_suggestions_block_list" => {
                self.update_query_suggestions_block_list(id, input).await
            }
            "query_suggestions" => self.update_query_suggestions(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kendra", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "faq" => self.delete_faq(id).await,
            "access_control_configuration" => self.delete_access_control_configuration(id).await,
            "data_source" => self.delete_data_source(id).await,
            "query_suggestions_config" => self.delete_query_suggestions_config(id).await,
            "index" => self.delete_index(id).await,
            "principal_mapping" => self.delete_principal_mapping(id).await,
            "featured_results_set" => self.delete_featured_results_set(id).await,
            "thesaurus" => self.delete_thesaurus(id).await,
            "experience" => self.delete_experience(id).await,
            "snapshots" => self.delete_snapshots(id).await,
            "query_suggestions_block_list" => self.delete_query_suggestions_block_list(id).await,
            "query_suggestions" => self.delete_query_suggestions(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kendra", resource_name
            ))),
    ...
    async fn create_faq(&self, input: ResourceInput) -> Result<ResourceOutput> {
    ...
    /// Read a faq resource
    async fn read_faq(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn update_faq(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let client_token = input.get_optional_string("client_token")?;
            let role_arn = input.get_string("role_arn")?;
    ...
    async fn delete_faq(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_access_control_configuration(
        ...
        self.provider.runtime().block_on(async {
            ...
            let index_id = input.get_string("index_id")?;
            let description = input.get_optional_string("description")?;
            let hierarchical_access_control_list =
                input.get_optional_string("hierarchical_access_control_list")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    hierarchical_access_control_list.unwrap_or_default(),
                )
                .with_field(
                    "access_control_list",
                    access_control_list.unwrap_or_default(),
                )
                .with_field("client_token", client_token.unwrap_or_default()))
        })
    }
    ...
    async fn read_access_control_configuration(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kendra_client
    ...
    async fn delete_access_control_configuration(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kendra_client
    ...
    async fn create_data_source(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            let custom_document_enrichment_configuration =
                input.get_optional_string("custom_document_enrichment_configuration")?;
            let index_id = input.get_string("index_id")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    custom_document_enrichment_configuration.unwrap_or_default(),
                )
                .with_field("index_id", index_id.unwrap_or_default())
    ...
    async fn create_query_suggestions_config(
        ...
        self.provider.runtime().block_on(async {
            // Extract input fields
            let attribute_suggestions_config =
                input.get_optional_string("attribute_suggestions_config")?;
            let minimum_query_count = input.get_optional_string("minimum_query_count")?;
            let query_log_look_back_window_in_days =
                input.get_optional_string("query_log_look_back_window_in_days")?;
            let include_queries_without_user_information =
                input.get_optional_string("include_queries_without_user_information")?;
            let index_id = input.get_string("index_id")?;
            let mode = input.get_optional_string("mode")?;
            let minimum_number_of_querying_users =
                input.get_optional_string("minimum_number_of_querying_users")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    attribute_suggestions_config.unwrap_or_default(),
                )
                .with_field(
                    "minimum_query_count",
                    minimum_query_count.unwrap_or_default(),
                )
                .with_field(
                    "query_log_look_back_window_in_days",
                    query_log_look_back_window_in_days.unwrap_or_default(),
                )
                .with_field(
                    "include_queries_without_user_information",
                    include_queries_without_user_information.unwrap_or_default(),
                )
                .with_field("index_id", index_id.unwrap_or_default())
                .with_field("mode", mode.unwrap_or_default())
                .with_field(
                    "minimum_number_of_querying_users",
                    minimum_number_of_querying_users.unwrap_or_default(),
                ))
        })
    }

    /// Read a query_suggestions_config resource
    async fn read_query_suggestions_config(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kendra_client
    ...
    async fn delete_query_suggestions_config(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kendra_client
    ...
    async fn create_index(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            let server_side_encryption_configuration =
                input.get_optional_string("server_side_encryption_configuration")?;
            let description = input.get_optional_string("description")?;
            let name = input.get_string("name")?;
            let client_token = input.get_optional_string("client_token")?;
            let user_context_policy = input.get_optional_string("user_context_policy")?;
            let user_group_resolution_configuration =
                input.get_optional_string("user_group_resolution_configuration")?;
            let user_token_configurations =
                input.get_optional_string("user_token_configurations")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "user_context_policy",
                    user_context_policy.unwrap_or_default(),
                )
                .with_field(
                    "user_group_resolution_configuration",
                    user_group_resolution_configuration.unwrap_or_default(),
                )
                .with_field(
                    "user_token_configurations",
                    user_token_configurations.unwrap_or_default(),
                ))
        })
    }
    ...
    async fn create_principal_mapping(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let index_id = input.get_string("index_id")?;
    ...
    async fn read_principal_mapping(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kendra_client
    ...
    async fn delete_principal_mapping(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kendra_client
    ...
    async fn create_featured_results_set(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    featured_results_set_name.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default())
    ...
    async fn read_featured_results_set(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kendra_client
    ...
    async fn delete_featured_results_set(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kendra_client
    ...
    async fn create_thesaurus(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_thesaurus(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kendra_client
    ...
    async fn update_thesaurus(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let source_s3_path = input.get_string("source_s3_path")?;
            let role_arn = input.get_string("role_arn")?;
    ...
    async fn delete_thesaurus(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kendra_client
    ...
    async fn create_experience(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let configuration = input.get_optional_string("configuration")?;
    ...
    async fn read_experience(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kendra_client
    ...
    async fn update_experience(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let configuration = input.get_optional_string("configuration")?;
            let index_id = input.get_string("index_id")?;
    ...
    async fn delete_experience(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kendra_client
    ...
    async fn read_query_suggestions_block_list(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kendra_client
    ...
    async fn delete_query_suggestions_block_list(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kendra_client
    ...
    async fn create_query_suggestions(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_query_suggestions(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kendra_client
    ...
    async fn delete_query_suggestions(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kendra_client
