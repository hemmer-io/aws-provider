...
impl<'a> Apigatewayv2Service<'a> {
    ...
    async fn create_route_responses(
        &self,
        input: ResourceInput,
    ) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
    ...
    async fn delete_route_responses(
        &self,
        id: &str,
    ) -> Result<()> {
        self.provider.runtime().block_on(async {
