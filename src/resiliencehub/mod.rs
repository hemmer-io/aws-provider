...
impl<'a> ResiliencehubService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "app_version_template" => {
                self.plan_app_version_template(current_state, desired_input)
                    .await
            }
            "app_version_app_component" => {
                self.plan_app_version_app_component(current_state, desired_input)
                    .await
            }
            "app_version_resource" => {
                self.plan_app_version_resource(current_state, desired_input)
                    .await
            }
            "metrics_export" => self.plan_metrics_export(current_state, desired_input).await,
            "draft_app_version_resources_import_status" => {
                self.plan_draft_app_version_resources_import_status(current_state, desired_input)
                    .await
            }
            "resource_grouping_recommendation_task" => {
                self.plan_resource_grouping_recommendation_task(current_state, desired_input)
                    .await
            }
            "app_input_source" => {
                self.plan_app_input_source(current_state, desired_input)
                    .await
            }
            "recommendation_template" => {
                self.plan_recommendation_template(current_state, desired_input)
                    .await
            }
            "draft_app_version_template" => {
                self.plan_draft_app_version_template(current_state, desired_input)
                    .await
            }
            "app" => self.plan_app(current_state, desired_input).await,
            "app_version_resources_resolution_status" => {
                self.plan_app_version_resources_resolution_status(current_state, desired_input)
                    .await
            }
            "resiliency_policy" => {
                self.plan_resiliency_policy(current_state, desired_input)
                    .await
            }
            "app_assessment" => self.plan_app_assessment(current_state, desired_input).await,
            "app_version" => self.plan_app_version(current_state, desired_input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "resiliencehub", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "app_version_template" => self.create_app_version_template(input).await,
            "app_version_app_component" => self.create_app_version_app_component(input).await,
            "app_version_resource" => self.create_app_version_resource(input).await,
            "metrics_export" => self.create_metrics_export(input).await,
            "draft_app_version_resources_import_status" => {
                self.create_draft_app_version_resources_import_status(input)
                    .await
            }
            "resource_grouping_recommendation_task" => {
                self.create_resource_grouping_recommendation_task(input)
                    .await
            }
            "app_input_source" => self.create_app_input_source(input).await,
            "recommendation_template" => self.create_recommendation_template(input).await,
            "draft_app_version_template" => self.create_draft_app_version_template(input).await,
            "app" => self.create_app(input).await,
            "app_version_resources_resolution_status" => {
                self.create_app_version_resources_resolution_status(input)
                    .await
            }
            "resiliency_policy" => self.create_resiliency_policy(input).await,
            "app_assessment" => self.create_app_assessment(input).await,
            "app_version" => self.create_app_version(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "resiliencehub", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "app_version_template" => self.read_app_version_template(id).await,
            "app_version_app_component" => self.read_app_version_app_component(id).await,
            "app_version_resource" => self.read_app_version_resource(id).await,
            "metrics_export" => self.read_metrics_export(id).await,
            "draft_app_version_resources_import_status" => {
                self.read_draft_app_version_resources_import_status(id)
                    .await
            }
            "resource_grouping_recommendation_task" => {
                self.read_resource_grouping_recommendation_task(id).await
            }
            "app_input_source" => self.read_app_input_source(id).await,
            "recommendation_template" => self.read_recommendation_template(id).await,
            "draft_app_version_template" => self.read_draft_app_version_template(id).await,
            "app" => self.read_app(id).await,
            "app_version_resources_resolution_status" => {
                self.read_app_version_resources_resolution_status(id).await
            }
            "resiliency_policy" => self.read_resiliency_policy(id).await,
            "app_assessment" => self.read_app_assessment(id).await,
            "app_version" => self.read_app_version(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "resiliencehub", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "app_version_template" => self.update_app_version_template(id, input).await,
            "app_version_app_component" => self.update_app_version_app_component(id, input).await,
            "app_version_resource" => self.update_app_version_resource(id, input).await,
            "metrics_export" => self.update_metrics_export(id, input).await,
            "draft_app_version_resources_import_status" => {
                self.update_draft_app_version_resources_import_status(id, input)
                    .await
            }
            "resource_grouping_recommendation_task" => {
                self.update_resource_grouping_recommendation_task(id, input)
                    .await
            }
            "app_input_source" => self.update_app_input_source(id, input).await,
            "recommendation_template" => self.update_recommendation_template(id, input).await,
            "draft_app_version_template" => self.update_draft_app_version_template(id, input).await,
            "app" => self.update_app(id, input).await,
            "app_version_resources_resolution_status" => {
                self.update_app_version_resources_resolution_status(id, input)
                    .await
            }
            "resiliency_policy" => self.update_resiliency_policy(id, input).await,
            "app_assessment" => self.update_app_assessment(id, input).await,
            "app_version" => self.update_app_version(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "resiliencehub", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "app_version_template" => self.delete_app_version_template(id).await,
            "app_version_app_component" => self.delete_app_version_app_component(id).await,
            "app_version_resource" => self.delete_app_version_resource(id).await,
            "metrics_export" => self.delete_metrics_export(id).await,
            "draft_app_version_resources_import_status" => {
                self.delete_draft_app_version_resources_import_status(id)
                    .await
            }
            "resource_grouping_recommendation_task" => {
                self.delete_resource_grouping_recommendation_task(id).await
            }
            "app_input_source" => self.delete_app_input_source(id).await,
            "recommendation_template" => self.delete_recommendation_template(id).await,
            "draft_app_version_template" => self.delete_draft_app_version_template(id).await,
            "app" => self.delete_app(id).await,
            "app_version_resources_resolution_status" => {
                self.delete_app_version_resources_resolution_status(id)
                    .await
            }
            "resiliency_policy" => self.delete_resiliency_policy(id).await,
            "app_assessment" => self.delete_app_assessment(id).await,
            "app_version" => self.delete_app_version(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "resiliencehub", resource_name
            ))),
    ...
    async fn create_app_version_template(&self, input: ResourceInput) -> Result<ResourceOutput> {
    ...
    async fn read_app_version_template(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_app_version_template(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn read_app_version_app_component(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.resiliencehub_client
    ...
    async fn delete_app_version_app_component(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.resiliencehub_client
    ...
    async fn create_app_version_resource(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "logical_resource_id",
                    logical_resource_id.unwrap_or_default(),
                )
                ...
                .with_field(
                    ...
                    physical_resource_id.unwrap_or_default(),
                )
                .with_field("client_token", client_token.unwrap_or_default())
    ...
    async fn read_app_version_resource(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.resiliencehub_client
    ...
    async fn delete_app_version_resource(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.resiliencehub_client
    ...
    async fn create_metrics_export(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_metrics_export(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.resiliencehub_client
    ...
    async fn delete_metrics_export(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.resiliencehub_client
    ...
    async fn delete_draft_app_version_resources_import_status(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.resiliencehub_client
    ...
    async fn read_resource_grouping_recommendation_task(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.resiliencehub_client
    ...
    async fn delete_resource_grouping_recommendation_task(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.resiliencehub_client
    ...
    async fn create_app_input_source(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_app_input_source(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.resiliencehub_client
    ...
    async fn delete_app_input_source(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.resiliencehub_client
    ...
    async fn create_recommendation_template(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    recommendation_types.unwrap_or_default(),
                )
    ...
    async fn read_recommendation_template(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.resiliencehub_client
    ...
    async fn delete_recommendation_template(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.resiliencehub_client
    ...
    async fn read_draft_app_version_template(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.resiliencehub_client
    ...
    async fn delete_draft_app_version_template(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.resiliencehub_client
    ...
    async fn create_app(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "event_subscriptions",
                    event_subscriptions.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default())
                .with_field("name", name.unwrap_or_default())
                .with_field(
                    "aws_application_arn",
                    aws_application_arn.unwrap_or_default(),
                )
                .with_field("client_token", client_token.unwrap_or_default())
                .with_field("permission_model", permission_model.unwrap_or_default())
                .with_field(
                    "assessment_schedule",
                    assessment_schedule.unwrap_or_default(),
                ))
        })
    }
    ...
    async fn delete_app_version_resources_resolution_status(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.resiliencehub_client
    ...
    async fn create_resiliency_policy(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let tags = input.get_optional_string("tags")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    data_location_constraint.unwrap_or_default(),
                ))
        })
    }

    /// Read a resiliency_policy resource
    async fn read_resiliency_policy(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.resiliencehub_client
    ...
    async fn delete_resiliency_policy(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.resiliencehub_client
    ...
    async fn create_app_assessment(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_app_assessment(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.resiliencehub_client
    ...
    async fn delete_app_assessment(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.resiliencehub_client
    ...
    async fn create_app_version(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let app_arn = input.get_string("app_arn")?;
    ...
    async fn read_app_version(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.resiliencehub_client
    ...
    async fn update_app_version(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let app_arn = input.get_string("app_arn")?;
            let additional_info = input.get_optional_string("additional_info")?;
    ...
    async fn delete_app_version(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.resiliencehub_client
