...
impl<'a> Personalize_eventsService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "items" => self.plan_items(current_state, desired_input).await,
            "action_interactions" => {
                self.plan_action_interactions(current_state, desired_input)
                    .await
            }
            "events" => self.plan_events(current_state, desired_input).await,
            "users" => self.plan_users(current_state, desired_input).await,
            "actions" => self.plan_actions(current_state, desired_input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "personalize_events", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "items" => self.create_items(input).await,
            "action_interactions" => self.create_action_interactions(input).await,
            "events" => self.create_events(input).await,
            "users" => self.create_users(input).await,
            "actions" => self.create_actions(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "personalize_events", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "items" => self.read_items(id).await,
            "action_interactions" => self.read_action_interactions(id).await,
            "events" => self.read_events(id).await,
            "users" => self.read_users(id).await,
            "actions" => self.read_actions(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "personalize_events", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "items" => self.update_items(id, input).await,
            "action_interactions" => self.update_action_interactions(id, input).await,
            "events" => self.update_events(id, input).await,
            "users" => self.update_users(id, input).await,
            "actions" => self.update_actions(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "personalize_events", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "items" => self.delete_items(id).await,
            "action_interactions" => self.delete_action_interactions(id).await,
            "events" => self.delete_events(id).await,
            "users" => self.delete_users(id).await,
            "actions" => self.delete_actions(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "personalize_events", resource_name
            ))),
    ...
    async fn create_items(&self, input: ResourceInput) -> Result<ResourceOutput> {
    ...
    async fn read_items(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn update_items(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let items = input.get_string("items")?;
            let dataset_arn = input.get_string("dataset_arn")?;
    ...
    async fn delete_items(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_action_interactions(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "action_interactions",
                    action_interactions.unwrap_or_default(),
                ))
        })
    }

    /// Read a action_interactions resource
    async fn read_action_interactions(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.personalize_events_client
    ...
    async fn delete_action_interactions(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.personalize_events_client
    ...
    async fn create_actions(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let dataset_arn = input.get_string("dataset_arn")?;
    ...
    async fn read_actions(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.personalize_events_client
    ...
    async fn update_actions(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let dataset_arn = input.get_string("dataset_arn")?;
            let actions = input.get_string("actions")?;
    ...
    async fn delete_actions(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.personalize_events_client
