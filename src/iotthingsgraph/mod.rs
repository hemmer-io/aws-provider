...
impl<'a> IotthingsgraphService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            ...
            "system_template_revisions" => {
                self.plan_system_template_revisions(current_state, desired_input)
                    .await
            }
            "upload_status" => self.plan_upload_status(current_state, desired_input).await,
            "system_instance" => {
                self.plan_system_instance(current_state, desired_input)
                    .await
            }
            "entities" => self.plan_entities(current_state, desired_input).await,
            "flow_template_revisions" => {
                self.plan_flow_template_revisions(current_state, desired_input)
                    .await
            }
            "flow_template" => self.plan_flow_template(current_state, desired_input).await,
            "system_template" => {
                self.plan_system_template(current_state, desired_input)
                    .await
            }
            "namespace_deletion_status" => {
                self.plan_namespace_deletion_status(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "iotthingsgraph", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            ...
            "system_template_revisions" => self.create_system_template_revisions(input).await,
            "upload_status" => self.create_upload_status(input).await,
            "system_instance" => self.create_system_instance(input).await,
            "entities" => self.create_entities(input).await,
            "flow_template_revisions" => self.create_flow_template_revisions(input).await,
            "flow_template" => self.create_flow_template(input).await,
            "system_template" => self.create_system_template(input).await,
            "namespace_deletion_status" => self.create_namespace_deletion_status(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "iotthingsgraph", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            ...
            "system_template_revisions" => self.read_system_template_revisions(id).await,
            "upload_status" => self.read_upload_status(id).await,
            "system_instance" => self.read_system_instance(id).await,
            "entities" => self.read_entities(id).await,
            "flow_template_revisions" => self.read_flow_template_revisions(id).await,
            "flow_template" => self.read_flow_template(id).await,
            "system_template" => self.read_system_template(id).await,
            "namespace_deletion_status" => self.read_namespace_deletion_status(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "iotthingsgraph", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            ...
            "system_template_revisions" => self.update_system_template_revisions(id, input).await,
            "upload_status" => self.update_upload_status(id, input).await,
            "system_instance" => self.update_system_instance(id, input).await,
            "entities" => self.update_entities(id, input).await,
            "flow_template_revisions" => self.update_flow_template_revisions(id, input).await,
            "flow_template" => self.update_flow_template(id, input).await,
            "system_template" => self.update_system_template(id, input).await,
            "namespace_deletion_status" => self.update_namespace_deletion_status(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "iotthingsgraph", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            ...
            "system_template_revisions" => self.delete_system_template_revisions(id).await,
            "upload_status" => self.delete_upload_status(id).await,
            "system_instance" => self.delete_system_instance(id).await,
            "entities" => self.delete_entities(id).await,
            "flow_template_revisions" => self.delete_flow_template_revisions(id).await,
            "flow_template" => self.delete_flow_template(id).await,
            "system_template" => self.delete_system_template(id).await,
            "namespace_deletion_status" => self.delete_namespace_deletion_status(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "iotthingsgraph", resource_name
            ))),
    ...
    async fn read_system_template_revisions(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.iotthingsgraph_client
    ...
    async fn delete_system_template_revisions(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.iotthingsgraph_client
    ...
    async fn create_upload_status(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_upload_status(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.iotthingsgraph_client
    ...
    async fn update_upload_status(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_upload_status(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.iotthingsgraph_client
    ...
    async fn create_system_instance(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    greengrass_group_name.unwrap_or_default(),
                )
                .with_field(
                    "flow_actions_role_arn",
                    flow_actions_role_arn.unwrap_or_default(),
                )
    ...
    async fn read_system_instance(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.iotthingsgraph_client
    ...
    async fn delete_system_instance(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.iotthingsgraph_client
    ...
    async fn create_entities(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_entities(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.iotthingsgraph_client
    ...
    async fn update_entities(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_entities(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.iotthingsgraph_client
    ...
    async fn create_flow_template_revisions(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_flow_template_revisions(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.iotthingsgraph_client
    ...
    async fn delete_flow_template_revisions(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.iotthingsgraph_client
    ...
    async fn create_flow_template(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let compatible_namespace_version =
                input.get_optional_string("compatible_namespace_version")?;
            let definition = input.get_string("definition")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    compatible_namespace_version.unwrap_or_default(),
                )
                .with_field("definition", definition.unwrap_or_default()))
        })
    }
    ...
    async fn read_flow_template(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.iotthingsgraph_client
    ...
    async fn update_flow_template(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let compatible_namespace_version =
                input.get_optional_string("compatible_namespace_version")?;
    ...
    async fn delete_flow_template(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.iotthingsgraph_client
    ...
    async fn create_system_template(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let compatible_namespace_version =
    ...
    async fn read_system_template(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.iotthingsgraph_client
    ...
    async fn delete_system_template(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.iotthingsgraph_client
    ...
    async fn read_namespace_deletion_status(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.iotthingsgraph_client
    ...
    async fn delete_namespace_deletion_status(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.iotthingsgraph_client
