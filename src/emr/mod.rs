...
impl<'a> EmrService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "studio_session_mapping" => {
                self.plan_studio_session_mapping(current_state, desired_input)
                    .await
            }
            "block_public_access_configuration" => {
                self.plan_block_public_access_configuration(current_state, desired_input)
                    .await
            }
            "notebook_execution" => {
                self.plan_notebook_execution(current_state, desired_input)
                    .await
            }
            "release_label" => self.plan_release_label(current_state, desired_input).await,
            "auto_termination_policy" => {
                self.plan_auto_termination_policy(current_state, desired_input)
                    .await
            }
            "persistent_app_ui_presigned_url" => {
                self.plan_persistent_app_ui_presigned_url(current_state, desired_input)
                    .await
            }
            "persistent_app_ui" => {
                self.plan_persistent_app_ui(current_state, desired_input)
                    .await
            }
            "cluster_session_credentials" => {
                self.plan_cluster_session_credentials(current_state, desired_input)
                    .await
            }
            "studio" => self.plan_studio(current_state, desired_input).await,
            "cluster" => self.plan_cluster(current_state, desired_input).await,
            "on_cluster_app_ui_presigned_url" => {
                self.plan_on_cluster_app_ui_presigned_url(current_state, desired_input)
                    .await
            }
            ...
            "job_flows" => self.plan_job_flows(current_state, desired_input).await,
            "auto_scaling_policy" => {
                self.plan_auto_scaling_policy(current_state, desired_input)
                    .await
            }
            "managed_scaling_policy" => {
                self.plan_managed_scaling_policy(current_state, desired_input)
                    .await
            }
            "step" => self.plan_step(current_state, desired_input).await,
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "studio_session_mapping" => self.create_studio_session_mapping(input).await,
            "block_public_access_configuration" => {
                self.create_block_public_access_configuration(input).await
            }
            "notebook_execution" => self.create_notebook_execution(input).await,
            "release_label" => self.create_release_label(input).await,
            "auto_termination_policy" => self.create_auto_termination_policy(input).await,
            "persistent_app_ui_presigned_url" => {
                self.create_persistent_app_ui_presigned_url(input).await
            }
            "persistent_app_ui" => self.create_persistent_app_ui(input).await,
            "cluster_session_credentials" => self.create_cluster_session_credentials(input).await,
            "studio" => self.create_studio(input).await,
            ...
            "job_flows" => self.create_job_flows(input).await,
            "auto_scaling_policy" => self.create_auto_scaling_policy(input).await,
            "managed_scaling_policy" => self.create_managed_scaling_policy(input).await,
            "step" => self.create_step(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "emr", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "studio_session_mapping" => self.read_studio_session_mapping(id).await,
            "block_public_access_configuration" => {
                self.read_block_public_access_configuration(id).await
            }
            "notebook_execution" => self.read_notebook_execution(id).await,
            "release_label" => self.read_release_label(id).await,
            "auto_termination_policy" => self.read_auto_termination_policy(id).await,
            "persistent_app_ui_presigned_url" => {
                self.read_persistent_app_ui_presigned_url(id).await
            }
            "persistent_app_ui" => self.read_persistent_app_ui(id).await,
            "cluster_session_credentials" => self.read_cluster_session_credentials(id).await,
            "studio" => self.read_studio(id).await,
            ...
            "job_flows" => self.read_job_flows(id).await,
            "auto_scaling_policy" => self.read_auto_scaling_policy(id).await,
            "managed_scaling_policy" => self.read_managed_scaling_policy(id).await,
            "step" => self.read_step(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "emr", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "studio_session_mapping" => self.update_studio_session_mapping(id, input).await,
            "block_public_access_configuration" => {
                self.update_block_public_access_configuration(id, input)
                    .await
            }
            "notebook_execution" => self.update_notebook_execution(id, input).await,
            "release_label" => self.update_release_label(id, input).await,
            "auto_termination_policy" => self.update_auto_termination_policy(id, input).await,
            "persistent_app_ui_presigned_url" => {
                self.update_persistent_app_ui_presigned_url(id, input).await
            }
            "persistent_app_ui" => self.update_persistent_app_ui(id, input).await,
            "cluster_session_credentials" => {
                self.update_cluster_session_credentials(id, input).await
            }
            "studio" => self.update_studio(id, input).await,
            ...
            "job_flows" => self.update_job_flows(id, input).await,
            "auto_scaling_policy" => self.update_auto_scaling_policy(id, input).await,
            "managed_scaling_policy" => self.update_managed_scaling_policy(id, input).await,
            "step" => self.update_step(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "emr", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "studio_session_mapping" => self.delete_studio_session_mapping(id).await,
            "block_public_access_configuration" => {
                self.delete_block_public_access_configuration(id).await
            }
            "notebook_execution" => self.delete_notebook_execution(id).await,
            "release_label" => self.delete_release_label(id).await,
            "auto_termination_policy" => self.delete_auto_termination_policy(id).await,
            "persistent_app_ui_presigned_url" => {
                self.delete_persistent_app_ui_presigned_url(id).await
            }
            "persistent_app_ui" => self.delete_persistent_app_ui(id).await,
            "cluster_session_credentials" => self.delete_cluster_session_credentials(id).await,
            "studio" => self.delete_studio(id).await,
            ...
            "job_flows" => self.delete_job_flows(id).await,
            "auto_scaling_policy" => self.delete_auto_scaling_policy(id).await,
            "managed_scaling_policy" => self.delete_managed_scaling_policy(id).await,
            "step" => self.delete_step(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "emr", resource_name
            ))),
    ...
    async fn create_studio_session_mapping(&self, input: ResourceInput) -> Result<ResourceOutput> {
    ...
    async fn read_studio_session_mapping(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_studio_session_mapping(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_block_public_access_configuration(
        ...
        self.provider.runtime().block_on(async {
            // Extract input fields
            let block_public_access_configuration =
                input.get_string("block_public_access_configuration")?;
            ...
            Ok(ResourceOutput::new().with_id("placeholder-id").with_field(
                "block_public_access_configuration",
                block_public_access_configuration.unwrap_or_default(),
            ))
        })
    }

    /// Read a block_public_access_configuration resource
    async fn read_block_public_access_configuration(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn delete_block_public_access_configuration(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
    ...
    async fn create_notebook_execution(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_notebook_execution(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn delete_notebook_execution(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
    ...
    async fn create_release_label(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_release_label(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn update_release_label(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_release_label(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
    ...
    async fn create_auto_termination_policy(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "auto_termination_policy",
                    auto_termination_policy.unwrap_or_default(),
                ))
        })
    }

    /// Read a auto_termination_policy resource
    async fn read_auto_termination_policy(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn delete_auto_termination_policy(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
    ...
    async fn read_persistent_app_ui_presigned_url(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn delete_persistent_app_ui_presigned_url(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
    ...
    async fn create_persistent_app_ui(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "target_resource_arn",
                    target_resource_arn.unwrap_or_default(),
                )
                .with_field(
                    "emr_containers_config",
                    emr_containers_config.unwrap_or_default(),
                )
    ...
    async fn read_persistent_app_ui(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn delete_persistent_app_ui(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
    ...
    async fn read_cluster_session_credentials(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn delete_cluster_session_credentials(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
    ...
    async fn create_studio(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            let idp_relay_state_parameter_name =
                input.get_optional_string("idp_relay_state_parameter_name")?;
            let trusted_identity_propagation_enabled =
                input.get_optional_string("trusted_identity_propagation_enabled")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    engine_security_group_id.unwrap_or_default(),
                )
                ...
                .with_field(
                    ...
                    workspace_security_group_id.unwrap_or_default(),
                )
                .with_field(
                    "default_s3_location",
                    default_s3_location.unwrap_or_default(),
                )
                .with_field(
                    "idp_relay_state_parameter_name",
                    idp_relay_state_parameter_name.unwrap_or_default(),
                )
                .with_field(
                    "trusted_identity_propagation_enabled",
                    trusted_identity_propagation_enabled.unwrap_or_default(),
                )
                .with_field(
                    "idc_user_assignment",
                    idc_user_assignment.unwrap_or_default(),
                )
    ...
    async fn read_studio(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn update_studio(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let engine_security_group_id = input.get_string("engine_security_group_id")?;
            let vpc_id = input.get_string("vpc_id")?;
    ...
    async fn delete_studio(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
    ...
    async fn read_on_cluster_app_ui_presigned_url(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn delete_on_cluster_app_ui_presigned_url(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
    ...
    async fn create_job_flows(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_job_flows(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn update_job_flows(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_job_flows(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
    ...
    async fn create_auto_scaling_policy(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let cluster_id = input.get_string("cluster_id")?;
    ...
    async fn read_auto_scaling_policy(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn delete_auto_scaling_policy(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
    ...
    async fn create_managed_scaling_policy(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let cluster_id = input.get_string("cluster_id")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "managed_scaling_policy",
                    managed_scaling_policy.unwrap_or_default(),
                ))
        })
    }

    /// Read a managed_scaling_policy resource
    async fn read_managed_scaling_policy(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn delete_managed_scaling_policy(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
    ...
    async fn create_step(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_step(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.emr_client
    ...
    async fn update_step(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_step(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.emr_client
