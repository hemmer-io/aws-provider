...
impl<'a> AthenaService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "prepared_statement" => {
                self.plan_prepared_statement(current_state, desired_input)
                    .await
            }
            "query_runtime_statistics" => {
                self.plan_query_runtime_statistics(current_state, desired_input)
                    .await
            }
            ...
            "capacity_assignment_configuration" => {
                self.plan_capacity_assignment_configuration(current_state, desired_input)
                    .await
            }
            "notebook" => self.plan_notebook(current_state, desired_input).await,
            "calculation_execution" => {
                self.plan_calculation_execution(current_state, desired_input)
                    .await
            }
            "notebook_metadata" => {
                self.plan_notebook_metadata(current_state, desired_input)
                    .await
            }
            ...
            "work_group" => self.plan_work_group(current_state, desired_input).await,
            "calculation_execution_code" => {
                self.plan_calculation_execution_code(current_state, desired_input)
                    .await
            }
            "data_catalog" => self.plan_data_catalog(current_state, desired_input).await,
            "presigned_notebook_url" => {
                self.plan_presigned_notebook_url(current_state, desired_input)
                    .await
            }
            "session_status" => self.plan_session_status(current_state, desired_input).await,
            "database" => self.plan_database(current_state, desired_input).await,
            "named_query" => self.plan_named_query(current_state, desired_input).await,
            "table_metadata" => self.plan_table_metadata(current_state, desired_input).await,
            "calculation_execution_status" => {
                self.plan_calculation_execution_status(current_state, desired_input)
                    .await
            }
            "query_execution" => {
                self.plan_query_execution(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "athena", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "prepared_statement" => self.create_prepared_statement(input).await,
            "query_runtime_statistics" => self.create_query_runtime_statistics(input).await,
            ...
            "calculation_execution" => self.create_calculation_execution(input).await,
            "notebook_metadata" => self.create_notebook_metadata(input).await,
            "capacity_reservation" => self.create_capacity_reservation(input).await,
            "work_group" => self.create_work_group(input).await,
            "calculation_execution_code" => self.create_calculation_execution_code(input).await,
            "data_catalog" => self.create_data_catalog(input).await,
            "presigned_notebook_url" => self.create_presigned_notebook_url(input).await,
            "session_status" => self.create_session_status(input).await,
            "database" => self.create_database(input).await,
            "named_query" => self.create_named_query(input).await,
            "table_metadata" => self.create_table_metadata(input).await,
            "calculation_execution_status" => self.create_calculation_execution_status(input).await,
            "query_execution" => self.create_query_execution(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "athena", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "prepared_statement" => self.read_prepared_statement(id).await,
            "query_runtime_statistics" => self.read_query_runtime_statistics(id).await,
            ...
            "calculation_execution" => self.read_calculation_execution(id).await,
            "notebook_metadata" => self.read_notebook_metadata(id).await,
            "capacity_reservation" => self.read_capacity_reservation(id).await,
            "work_group" => self.read_work_group(id).await,
            "calculation_execution_code" => self.read_calculation_execution_code(id).await,
            "data_catalog" => self.read_data_catalog(id).await,
            "presigned_notebook_url" => self.read_presigned_notebook_url(id).await,
            "session_status" => self.read_session_status(id).await,
            "database" => self.read_database(id).await,
            "named_query" => self.read_named_query(id).await,
            "table_metadata" => self.read_table_metadata(id).await,
            "calculation_execution_status" => self.read_calculation_execution_status(id).await,
            "query_execution" => self.read_query_execution(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "athena", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "prepared_statement" => self.update_prepared_statement(id, input).await,
            "query_runtime_statistics" => self.update_query_runtime_statistics(id, input).await,
            "session" => self.update_session(id, input).await,
            "query_results" => self.update_query_results(id, input).await,
            "capacity_assignment_configuration" => {
                self.update_capacity_assignment_configuration(id, input)
                    .await
            }
            "notebook" => self.update_notebook(id, input).await,
            "calculation_execution" => self.update_calculation_execution(id, input).await,
            "notebook_metadata" => self.update_notebook_metadata(id, input).await,
            "capacity_reservation" => self.update_capacity_reservation(id, input).await,
            "work_group" => self.update_work_group(id, input).await,
            "calculation_execution_code" => self.update_calculation_execution_code(id, input).await,
            "data_catalog" => self.update_data_catalog(id, input).await,
            "presigned_notebook_url" => self.update_presigned_notebook_url(id, input).await,
            "session_status" => self.update_session_status(id, input).await,
            "database" => self.update_database(id, input).await,
            "named_query" => self.update_named_query(id, input).await,
            "table_metadata" => self.update_table_metadata(id, input).await,
            "calculation_execution_status" => {
                self.update_calculation_execution_status(id, input).await
            }
            "query_execution" => self.update_query_execution(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "athena", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "prepared_statement" => self.delete_prepared_statement(id).await,
            "query_runtime_statistics" => self.delete_query_runtime_statistics(id).await,
            ...
            "calculation_execution" => self.delete_calculation_execution(id).await,
            "notebook_metadata" => self.delete_notebook_metadata(id).await,
            "capacity_reservation" => self.delete_capacity_reservation(id).await,
            "work_group" => self.delete_work_group(id).await,
            "calculation_execution_code" => self.delete_calculation_execution_code(id).await,
            "data_catalog" => self.delete_data_catalog(id).await,
            "presigned_notebook_url" => self.delete_presigned_notebook_url(id).await,
            "session_status" => self.delete_session_status(id).await,
            "database" => self.delete_database(id).await,
            "named_query" => self.delete_named_query(id).await,
            "table_metadata" => self.delete_table_metadata(id).await,
            "calculation_execution_status" => self.delete_calculation_execution_status(id).await,
            "query_execution" => self.delete_query_execution(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "athena", resource_name
            ))),
    ...
    async fn create_prepared_statement(&self, input: ResourceInput) -> Result<ResourceOutput> {
    ...
    async fn read_prepared_statement(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_prepared_statement(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn read_query_runtime_statistics(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn delete_query_runtime_statistics(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    capacity_assignments.unwrap_or_default(),
                )
                .with_field(
                    "capacity_reservation_name",
                    capacity_reservation_name.unwrap_or_default(),
                ))
        })
    }

    /// Read a capacity_assignment_configuration resource
    async fn read_capacity_assignment_configuration(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn delete_capacity_assignment_configuration(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
    ...
    async fn create_calculation_execution(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_calculation_execution(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn delete_calculation_execution(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
    ...
    async fn create_notebook_metadata(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let name = input.get_string("name")?;
    ...
    async fn read_notebook_metadata(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn delete_notebook_metadata(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
    ...
    async fn create_work_group(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let description = input.get_optional_string("description")?;
    ...
    async fn read_work_group(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn update_work_group(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let description = input.get_optional_string("description")?;
            let configuration = input.get_optional_string("configuration")?;
    ...
    async fn delete_work_group(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
    ...
    async fn read_calculation_execution_code(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn delete_calculation_execution_code(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
    ...
    async fn create_data_catalog(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_data_catalog(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn update_data_catalog(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let r#type = input.get_string("type")?;
            let parameters = input.get_optional_string("parameters")?;
    ...
    async fn delete_data_catalog(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
    ...
    async fn create_presigned_notebook_url(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_presigned_notebook_url(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn delete_presigned_notebook_url(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
    ...
    async fn create_session_status(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_session_status(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn delete_session_status(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
    ...
    async fn create_named_query(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_named_query(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn update_named_query(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let query_string = input.get_string("query_string")?;
            let description = input.get_optional_string("description")?;
    ...
    async fn delete_named_query(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
    ...
    async fn create_table_metadata(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_table_metadata(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn delete_table_metadata(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
    ...
    async fn read_calculation_execution_status(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn delete_calculation_execution_status(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
    ...
    async fn create_query_execution(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_query_execution(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.athena_client
    ...
    async fn delete_query_execution(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.athena_client
