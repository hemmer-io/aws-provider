...
impl<'a> Macie2Service<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            ...
            "usage_totals" => self.plan_usage_totals(current_state, desired_input).await,
            "master_account" => self.plan_master_account(current_state, desired_input).await,
            "allow_list" => self.plan_allow_list(current_state, desired_input).await,
            "resource_profile_detections" => {
                self.plan_resource_profile_detections(current_state, desired_input)
                    .await
            }
            "finding_statistics" => {
                self.plan_finding_statistics(current_state, desired_input)
                    .await
            }
            "findings_publication_configuration" => {
                self.plan_findings_publication_configuration(current_state, desired_input)
                    .await
            }
            "macie_session" => self.plan_macie_session(current_state, desired_input).await,
            "sensitive_data_occurrences_availability" => {
                self.plan_sensitive_data_occurrences_availability(current_state, desired_input)
                    .await
            }
            "classification_scope" => {
                self.plan_classification_scope(current_state, desired_input)
                    .await
            }
            "resource_profile" => {
                self.plan_resource_profile(current_state, desired_input)
                    .await
            }
            ...
            "automated_discovery_configuration" => {
                self.plan_automated_discovery_configuration(current_state, desired_input)
                    .await
            }
            "reveal_configuration" => {
                self.plan_reveal_configuration(current_state, desired_input)
                    .await
            }
            "bucket_statistics" => {
                self.plan_bucket_statistics(current_state, desired_input)
                    .await
            }
            "custom_data_identifier" => {
                self.plan_custom_data_identifier(current_state, desired_input)
                    .await
            }
            "sensitive_data_occurrences" => {
                self.plan_sensitive_data_occurrences(current_state, desired_input)
                    .await
            }
            "member_session" => self.plan_member_session(current_state, desired_input).await,
            "organization_configuration" => {
                self.plan_organization_configuration(current_state, desired_input)
                    .await
            }
            "classification_job" => {
                self.plan_classification_job(current_state, desired_input)
                    .await
            }
            "findings_filter" => {
                self.plan_findings_filter(current_state, desired_input)
                    .await
            }
            "classification_export_configuration" => {
                self.plan_classification_export_configuration(current_state, desired_input)
                    .await
            }
            "sensitivity_inspection_template" => {
                self.plan_sensitivity_inspection_template(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "macie2", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            ...
            "usage_totals" => self.create_usage_totals(input).await,
            "master_account" => self.create_master_account(input).await,
            "allow_list" => self.create_allow_list(input).await,
            "resource_profile_detections" => self.create_resource_profile_detections(input).await,
            "finding_statistics" => self.create_finding_statistics(input).await,
            "findings_publication_configuration" => {
                self.create_findings_publication_configuration(input).await
            }
            "macie_session" => self.create_macie_session(input).await,
            "sensitive_data_occurrences_availability" => {
                self.create_sensitive_data_occurrences_availability(input)
                    .await
            }
            "classification_scope" => self.create_classification_scope(input).await,
            "resource_profile" => self.create_resource_profile(input).await,
            ...
            "reveal_configuration" => self.create_reveal_configuration(input).await,
            "bucket_statistics" => self.create_bucket_statistics(input).await,
            "custom_data_identifier" => self.create_custom_data_identifier(input).await,
            "sensitive_data_occurrences" => self.create_sensitive_data_occurrences(input).await,
            "member_session" => self.create_member_session(input).await,
            "organization_configuration" => self.create_organization_configuration(input).await,
            "classification_job" => self.create_classification_job(input).await,
            "findings_filter" => self.create_findings_filter(input).await,
            "classification_export_configuration" => {
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            ...
            "usage_totals" => self.read_usage_totals(id).await,
            "master_account" => self.read_master_account(id).await,
            "allow_list" => self.read_allow_list(id).await,
            "resource_profile_detections" => self.read_resource_profile_detections(id).await,
            "finding_statistics" => self.read_finding_statistics(id).await,
            "findings_publication_configuration" => {
                self.read_findings_publication_configuration(id).await
            }
            "macie_session" => self.read_macie_session(id).await,
            "sensitive_data_occurrences_availability" => {
                self.read_sensitive_data_occurrences_availability(id).await
            }
            "classification_scope" => self.read_classification_scope(id).await,
            "resource_profile" => self.read_resource_profile(id).await,
            ...
            "reveal_configuration" => self.read_reveal_configuration(id).await,
            "bucket_statistics" => self.read_bucket_statistics(id).await,
            "custom_data_identifier" => self.read_custom_data_identifier(id).await,
            "sensitive_data_occurrences" => self.read_sensitive_data_occurrences(id).await,
            "member_session" => self.read_member_session(id).await,
            "organization_configuration" => self.read_organization_configuration(id).await,
            "classification_job" => self.read_classification_job(id).await,
            "findings_filter" => self.read_findings_filter(id).await,
            "classification_export_configuration" => {
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            ...
            "usage_totals" => self.update_usage_totals(id, input).await,
            "master_account" => self.update_master_account(id, input).await,
            "allow_list" => self.update_allow_list(id, input).await,
            "resource_profile_detections" => {
                self.update_resource_profile_detections(id, input).await
            }
            "finding_statistics" => self.update_finding_statistics(id, input).await,
            "findings_publication_configuration" => {
                self.update_findings_publication_configuration(id, input)
                    .await
            }
            "macie_session" => self.update_macie_session(id, input).await,
            "sensitive_data_occurrences_availability" => {
                self.update_sensitive_data_occurrences_availability(id, input)
                    .await
            }
            "classification_scope" => self.update_classification_scope(id, input).await,
            "resource_profile" => self.update_resource_profile(id, input).await,
            ...
            "automated_discovery_configuration" => {
                self.update_automated_discovery_configuration(id, input)
                    .await
            }
            "reveal_configuration" => self.update_reveal_configuration(id, input).await,
            "bucket_statistics" => self.update_bucket_statistics(id, input).await,
            "custom_data_identifier" => self.update_custom_data_identifier(id, input).await,
            "sensitive_data_occurrences" => self.update_sensitive_data_occurrences(id, input).await,
            "member_session" => self.update_member_session(id, input).await,
            "organization_configuration" => self.update_organization_configuration(id, input).await,
            "classification_job" => self.update_classification_job(id, input).await,
            "findings_filter" => self.update_findings_filter(id, input).await,
            "classification_export_configuration" => {
                self.update_classification_export_configuration(id, input)
                    .await
            }
            "sensitivity_inspection_template" => {
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            ...
            "usage_totals" => self.delete_usage_totals(id).await,
            "master_account" => self.delete_master_account(id).await,
            "allow_list" => self.delete_allow_list(id).await,
            "resource_profile_detections" => self.delete_resource_profile_detections(id).await,
            "finding_statistics" => self.delete_finding_statistics(id).await,
            "findings_publication_configuration" => {
                self.delete_findings_publication_configuration(id).await
            }
            "macie_session" => self.delete_macie_session(id).await,
            "sensitive_data_occurrences_availability" => {
                self.delete_sensitive_data_occurrences_availability(id)
                    .await
            }
            "classification_scope" => self.delete_classification_scope(id).await,
            "resource_profile" => self.delete_resource_profile(id).await,
            ...
            "reveal_configuration" => self.delete_reveal_configuration(id).await,
            "bucket_statistics" => self.delete_bucket_statistics(id).await,
            "custom_data_identifier" => self.delete_custom_data_identifier(id).await,
            "sensitive_data_occurrences" => self.delete_sensitive_data_occurrences(id).await,
            "member_session" => self.delete_member_session(id).await,
            "organization_configuration" => self.delete_organization_configuration(id).await,
            "classification_job" => self.delete_classification_job(id).await,
            "findings_filter" => self.delete_findings_filter(id).await,
            "classification_export_configuration" => {
    ...
    async fn create_usage_totals(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_usage_totals(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn update_usage_totals(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_usage_totals(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_allow_list(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_allow_list(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn update_allow_list(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let description = input.get_optional_string("description")?;
            let criteria = input.get_string("criteria")?;
    ...
    async fn delete_allow_list(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_resource_profile_detections(
        ...
        self.provider.runtime().block_on(async {
            // Extract input fields
            let suppress_data_identifiers =
                input.get_optional_string("suppress_data_identifiers")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    suppress_data_identifiers.unwrap_or_default(),
                )
    ...
    async fn read_resource_profile_detections(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_resource_profile_detections(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_finding_statistics(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_finding_statistics(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_finding_statistics(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_findings_publication_configuration(
        ...
        self.provider.runtime().block_on(async {
            // Extract input fields
            let security_hub_configuration =
                input.get_optional_string("security_hub_configuration")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    security_hub_configuration.unwrap_or_default(),
                )
                .with_field("client_token", client_token.unwrap_or_default()))
        })
    }
    ...
    async fn read_findings_publication_configuration(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_findings_publication_configuration(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_macie_session(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_macie_session(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn update_macie_session(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let finding_publishing_frequency =
                input.get_optional_string("finding_publishing_frequency")?;
    ...
    async fn delete_macie_session(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn delete_sensitive_data_occurrences_availability(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_classification_scope(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_classification_scope(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_classification_scope(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_resource_profile(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let sensitivity_score_override =
                input.get_optional_string("sensitivity_score_override")?;
            let resource_arn = input.get_string("resource_arn")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    sensitivity_score_override.unwrap_or_default(),
                )
                .with_field("resource_arn", resource_arn.unwrap_or_default()))
        })
    }
    ...
    async fn read_resource_profile(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_resource_profile(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn read_automated_discovery_configuration(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_automated_discovery_configuration(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_reveal_configuration(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    retrieval_configuration.unwrap_or_default(),
                )
    ...
    async fn read_reveal_configuration(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_reveal_configuration(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_bucket_statistics(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_bucket_statistics(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_bucket_statistics(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_custom_data_identifier(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let tags = input.get_optional_string("tags")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    maximum_match_distance.unwrap_or_default(),
                )
    ...
    async fn read_custom_data_identifier(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_custom_data_identifier(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn read_sensitive_data_occurrences(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_sensitive_data_occurrences(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_member_session(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let status = input.get_string("status")?;
    ...
    async fn read_member_session(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_member_session(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_classification_job(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let initial_run = input.get_optional_string("initial_run")?;
            let managed_data_identifier_ids =
                input.get_optional_string("managed_data_identifier_ids")?;
            let client_token = input.get_string("client_token")?;
            let job_type = input.get_string("job_type")?;
            let custom_data_identifier_ids =
                input.get_optional_string("custom_data_identifier_ids")?;
            let managed_data_identifier_selector =
                input.get_optional_string("managed_data_identifier_selector")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    managed_data_identifier_ids.unwrap_or_default(),
                )
                .with_field("client_token", client_token.unwrap_or_default())
                .with_field("job_type", job_type.unwrap_or_default())
                .with_field(
                    "custom_data_identifier_ids",
                    custom_data_identifier_ids.unwrap_or_default(),
                )
                .with_field(
                    "managed_data_identifier_selector",
                    managed_data_identifier_selector.unwrap_or_default(),
                )
                ...
                .with_field(
                    "sampling_percentage",
                    sampling_percentage.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default())
                .with_field("description", description.unwrap_or_default()))
        })
    ...
    async fn read_classification_job(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_classification_job(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn create_findings_filter(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let client_token = input.get_optional_string("client_token")?;
    ...
    async fn read_findings_filter(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_findings_filter(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn read_classification_export_configuration(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_classification_export_configuration(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
    ...
    async fn read_sensitivity_inspection_template(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.macie2_client
    ...
    async fn delete_sensitivity_inspection_template(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.macie2_client
