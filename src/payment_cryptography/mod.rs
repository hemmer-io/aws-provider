...
impl<'a> Payment_cryptographyService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "certificate_signing_request" => {
                self.plan_certificate_signing_request(current_state, desired_input)
                    .await
            }
            "default_key_replication_regions" => {
                self.plan_default_key_replication_regions(current_state, desired_input)
                    .await
            }
            "parameters_for_export" => {
                self.plan_parameters_for_export(current_state, desired_input)
                    .await
            }
            ...
            "public_key_certificate" => {
                self.plan_public_key_certificate(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "payment_cryptography", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "certificate_signing_request" => self.create_certificate_signing_request(input).await,
            "default_key_replication_regions" => {
                self.create_default_key_replication_regions(input).await
            }
            "parameters_for_export" => self.create_parameters_for_export(input).await,
            "parameters_for_import" => self.create_parameters_for_import(input).await,
            "public_key_certificate" => self.create_public_key_certificate(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "payment_cryptography", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "certificate_signing_request" => self.read_certificate_signing_request(id).await,
            "default_key_replication_regions" => {
                self.read_default_key_replication_regions(id).await
            }
            "parameters_for_export" => self.read_parameters_for_export(id).await,
            "parameters_for_import" => self.read_parameters_for_import(id).await,
            "public_key_certificate" => self.read_public_key_certificate(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "payment_cryptography", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            ...
            "parameters_for_export" => self.update_parameters_for_export(id, input).await,
            "parameters_for_import" => self.update_parameters_for_import(id, input).await,
            "public_key_certificate" => self.update_public_key_certificate(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "payment_cryptography", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "certificate_signing_request" => self.delete_certificate_signing_request(id).await,
            "default_key_replication_regions" => {
                self.delete_default_key_replication_regions(id).await
            }
            "parameters_for_export" => self.delete_parameters_for_export(id).await,
            "parameters_for_import" => self.delete_parameters_for_import(id).await,
            "public_key_certificate" => self.delete_public_key_certificate(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "payment_cryptography", resource_name
            ))),
    ...
    async fn read_certificate_signing_request(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_certificate_signing_request(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn read_default_key_replication_regions(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.payment_cryptography_client
    ...
    async fn delete_default_key_replication_regions(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.payment_cryptography_client
    ...
    async fn create_parameters_for_export(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_parameters_for_export(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.payment_cryptography_client
    ...
    async fn delete_parameters_for_export(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.payment_cryptography_client
    ...
    async fn create_public_key_certificate(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_public_key_certificate(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.payment_cryptography_client
    ...
    async fn delete_public_key_certificate(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.payment_cryptography_client
