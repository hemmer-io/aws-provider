...
impl<'a> ForecastService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "forecast" => self.plan_forecast(current_state, desired_input).await,
            "explainability_export" => {
                self.plan_explainability_export(current_state, desired_input)
                    .await
            }
            "monitor" => self.plan_monitor(current_state, desired_input).await,
            "resource_tree" => self.plan_resource_tree(current_state, desired_input).await,
            "dataset_import_job" => {
                self.plan_dataset_import_job(current_state, desired_input)
                    .await
            }
            "predictor_backtest_export_job" => {
                self.plan_predictor_backtest_export_job(current_state, desired_input)
                    .await
            }
            "dataset" => self.plan_dataset(current_state, desired_input).await,
            "predictor" => self.plan_predictor(current_state, desired_input).await,
            "what_if_forecast" => {
                self.plan_what_if_forecast(current_state, desired_input)
                    .await
            }
            "dataset_group" => self.plan_dataset_group(current_state, desired_input).await,
            "what_if_forecast_export" => {
                self.plan_what_if_forecast_export(current_state, desired_input)
                    .await
            }
            "accuracy_metrics" => {
                self.plan_accuracy_metrics(current_state, desired_input)
                    .await
            }
            "what_if_analysis" => {
                self.plan_what_if_analysis(current_state, desired_input)
                    .await
            }
            "auto_predictor" => self.plan_auto_predictor(current_state, desired_input).await,
            "forecast_export_job" => {
                self.plan_forecast_export_job(current_state, desired_input)
                    .await
            }
            "explainability" => self.plan_explainability(current_state, desired_input).await,
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "forecast" => self.create_forecast(input).await,
            "explainability_export" => self.create_explainability_export(input).await,
            "monitor" => self.create_monitor(input).await,
            "resource_tree" => self.create_resource_tree(input).await,
            "dataset_import_job" => self.create_dataset_import_job(input).await,
            "predictor_backtest_export_job" => {
                self.create_predictor_backtest_export_job(input).await
            }
            "dataset" => self.create_dataset(input).await,
            "predictor" => self.create_predictor(input).await,
            "what_if_forecast" => self.create_what_if_forecast(input).await,
            "dataset_group" => self.create_dataset_group(input).await,
            "what_if_forecast_export" => self.create_what_if_forecast_export(input).await,
            "accuracy_metrics" => self.create_accuracy_metrics(input).await,
            "what_if_analysis" => self.create_what_if_analysis(input).await,
            "auto_predictor" => self.create_auto_predictor(input).await,
            "forecast_export_job" => self.create_forecast_export_job(input).await,
            "explainability" => self.create_explainability(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "forecast", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "forecast" => self.read_forecast(id).await,
            "explainability_export" => self.read_explainability_export(id).await,
            "monitor" => self.read_monitor(id).await,
            "resource_tree" => self.read_resource_tree(id).await,
            "dataset_import_job" => self.read_dataset_import_job(id).await,
            "predictor_backtest_export_job" => self.read_predictor_backtest_export_job(id).await,
            "dataset" => self.read_dataset(id).await,
            "predictor" => self.read_predictor(id).await,
            "what_if_forecast" => self.read_what_if_forecast(id).await,
            "dataset_group" => self.read_dataset_group(id).await,
            "what_if_forecast_export" => self.read_what_if_forecast_export(id).await,
            "accuracy_metrics" => self.read_accuracy_metrics(id).await,
            "what_if_analysis" => self.read_what_if_analysis(id).await,
            "auto_predictor" => self.read_auto_predictor(id).await,
            "forecast_export_job" => self.read_forecast_export_job(id).await,
            "explainability" => self.read_explainability(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "forecast", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "forecast" => self.update_forecast(id, input).await,
            "explainability_export" => self.update_explainability_export(id, input).await,
            "monitor" => self.update_monitor(id, input).await,
            "resource_tree" => self.update_resource_tree(id, input).await,
            "dataset_import_job" => self.update_dataset_import_job(id, input).await,
            "predictor_backtest_export_job" => {
                self.update_predictor_backtest_export_job(id, input).await
            }
            "dataset" => self.update_dataset(id, input).await,
            "predictor" => self.update_predictor(id, input).await,
            "what_if_forecast" => self.update_what_if_forecast(id, input).await,
            "dataset_group" => self.update_dataset_group(id, input).await,
            "what_if_forecast_export" => self.update_what_if_forecast_export(id, input).await,
            "accuracy_metrics" => self.update_accuracy_metrics(id, input).await,
            "what_if_analysis" => self.update_what_if_analysis(id, input).await,
            "auto_predictor" => self.update_auto_predictor(id, input).await,
            "forecast_export_job" => self.update_forecast_export_job(id, input).await,
            "explainability" => self.update_explainability(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "forecast", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "forecast" => self.delete_forecast(id).await,
            "explainability_export" => self.delete_explainability_export(id).await,
            "monitor" => self.delete_monitor(id).await,
            "resource_tree" => self.delete_resource_tree(id).await,
            "dataset_import_job" => self.delete_dataset_import_job(id).await,
            "predictor_backtest_export_job" => self.delete_predictor_backtest_export_job(id).await,
            "dataset" => self.delete_dataset(id).await,
            "predictor" => self.delete_predictor(id).await,
            "what_if_forecast" => self.delete_what_if_forecast(id).await,
            "dataset_group" => self.delete_dataset_group(id).await,
            "what_if_forecast_export" => self.delete_what_if_forecast_export(id).await,
            "accuracy_metrics" => self.delete_accuracy_metrics(id).await,
            "what_if_analysis" => self.delete_what_if_analysis(id).await,
            "auto_predictor" => self.delete_auto_predictor(id).await,
            "forecast_export_job" => self.delete_forecast_export_job(id).await,
            "explainability" => self.delete_explainability(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "forecast", resource_name
            ))),
    ...
    async fn create_forecast(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    time_series_selector.unwrap_or_default(),
                )
    ...
    async fn read_forecast(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn update_forecast(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let predictor_arn = input.get_string("predictor_arn")?;
            let forecast_types = input.get_optional_string("forecast_types")?;
    ...
    async fn delete_forecast(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_explainability_export(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    explainability_export_name.unwrap_or_default(),
                )
    ...
    async fn read_explainability_export(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn delete_explainability_export(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
    ...
    async fn create_monitor(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_monitor(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn update_monitor(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let resource_arn = input.get_string("resource_arn")?;
            let tags = input.get_optional_string("tags")?;
    ...
    async fn create_resource_tree(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_resource_tree(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn update_resource_tree(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_resource_tree(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
    ...
    async fn create_dataset_import_job(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            let use_geolocation_for_time_zone =
                input.get_optional_string("use_geolocation_for_time_zone")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    dataset_import_job_name.unwrap_or_default(),
                )
                .with_field(
                    "use_geolocation_for_time_zone",
                    use_geolocation_for_time_zone.unwrap_or_default(),
                )
    ...
    async fn read_dataset_import_job(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn delete_dataset_import_job(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
    ...
    async fn create_predictor_backtest_export_job(
        ...
        self.provider.runtime().block_on(async {
            ...
            let format = input.get_optional_string("format")?;
            let predictor_arn = input.get_string("predictor_arn")?;
            let predictor_backtest_export_job_name =
                input.get_string("predictor_backtest_export_job_name")?;
            let destination = input.get_string("destination")?;
            let tags = input.get_optional_string("tags")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    predictor_backtest_export_job_name.unwrap_or_default(),
                )
                .with_field("destination", destination.unwrap_or_default())
                .with_field("tags", tags.unwrap_or_default()))
        })
    ...
    async fn read_predictor_backtest_export_job(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn delete_predictor_backtest_export_job(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
    ...
    async fn create_predictor(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            let auto_ml_override_strategy =
                input.get_optional_string("auto_ml_override_strategy")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    featurization_config.unwrap_or_default(),
                )
                ...
                .with_field(
                    "optimization_metric",
                    optimization_metric.unwrap_or_default(),
                )
                .with_field("encryption_config", encryption_config.unwrap_or_default())
                .with_field("forecast_types", forecast_types.unwrap_or_default())
                .with_field("tags", tags.unwrap_or_default())
                ...
                .with_field(
                    ...
                    auto_ml_override_strategy.unwrap_or_default(),
                )
                .with_field("predictor_name", predictor_name.unwrap_or_default())
                .with_field(
                    "training_parameters",
                    training_parameters.unwrap_or_default(),
                )
                .with_field(
                    "evaluation_parameters",
                    evaluation_parameters.unwrap_or_default(),
                ))
        })
    }

    /// Read a predictor resource
    async fn read_predictor(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn update_predictor(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let featurization_config = input.get_string("featurization_config")?;
            let perform_hpo = input.get_optional_string("perform_hpo")?;
    ...
    async fn delete_predictor(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
    ...
    async fn create_what_if_forecast(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let what_if_analysis_arn = input.get_string("what_if_analysis_arn")?;
            let tags = input.get_optional_string("tags")?;
            let time_series_transformations =
                input.get_optional_string("time_series_transformations")?;
            let time_series_replacements_data_source =
                input.get_optional_string("time_series_replacements_data_source")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    what_if_analysis_arn.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default())
                .with_field(
                    "time_series_transformations",
                    time_series_transformations.unwrap_or_default(),
                )
                .with_field(
                    "time_series_replacements_data_source",
                    time_series_replacements_data_source.unwrap_or_default(),
                )
                .with_field(
                    "what_if_forecast_name",
                    what_if_forecast_name.unwrap_or_default(),
                ))
        })
    }

    /// Read a what_if_forecast resource
    async fn read_what_if_forecast(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn delete_what_if_forecast(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
    ...
    async fn create_dataset_group(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let domain = input.get_string("domain")?;
    ...
    async fn read_dataset_group(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn update_dataset_group(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let domain = input.get_string("domain")?;
            let dataset_group_name = input.get_string("dataset_group_name")?;
    ...
    async fn delete_dataset_group(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
    ...
    async fn create_what_if_forecast_export(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    what_if_forecast_export_name.unwrap_or_default(),
                )
                .with_field(
                    "what_if_forecast_arns",
                    what_if_forecast_arns.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default())
                .with_field("destination", destination.unwrap_or_default())
                .with_field("format", format.unwrap_or_default()))
    ...
    async fn read_what_if_forecast_export(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn delete_what_if_forecast_export(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
    ...
    async fn create_accuracy_metrics(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_accuracy_metrics(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn delete_accuracy_metrics(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
    ...
    async fn create_what_if_analysis(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    what_if_analysis_name.unwrap_or_default(),
                ))
        })
    }

    /// Read a what_if_analysis resource
    async fn read_what_if_analysis(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn delete_what_if_analysis(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
    ...
    async fn create_auto_predictor(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let encryption_config = input.get_optional_string("encryption_config")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "forecast_dimensions",
                    forecast_dimensions.unwrap_or_default(),
                )
                .with_field("forecast_types", forecast_types.unwrap_or_default())
                ...
                .with_field(
                    ...
                    time_alignment_boundary.unwrap_or_default(),
                )
                .with_field("data_config", data_config.unwrap_or_default())
                .with_field(
                    "reference_predictor_arn",
                    reference_predictor_arn.unwrap_or_default(),
                )
                .with_field(
                    "optimization_metric",
                    optimization_metric.unwrap_or_default(),
    ...
    async fn read_auto_predictor(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn delete_auto_predictor(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
    ...
    async fn create_forecast_export_job(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let destination = input.get_string("destination")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    forecast_export_job_name.unwrap_or_default(),
                )
                .with_field("format", format.unwrap_or_default()))
        })
    }
    ...
    async fn read_forecast_export_job(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn delete_forecast_export_job(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
    ...
    async fn create_explainability(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "explainability_name",
                    explainability_name.unwrap_or_default(),
                )
                .with_field("resource_arn", resource_arn.unwrap_or_default())
                .with_field(
                    "explainability_config",
                    explainability_config.unwrap_or_default(),
                )
                .with_field(
                    "enable_visualization",
                    enable_visualization.unwrap_or_default(),
                ))
        })
    }

    /// Read a explainability resource
    async fn read_explainability(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.forecast_client
    ...
    async fn delete_explainability(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.forecast_client
