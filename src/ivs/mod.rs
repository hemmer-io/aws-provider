...
impl<'a> IvsService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            ...
            "stream_key" => self.plan_stream_key(current_state, desired_input).await,
            "playback_restriction_policy" => {
                self.plan_playback_restriction_policy(current_state, desired_input)
                    .await
            }
            "stream" => self.plan_stream(current_state, desired_input).await,
            "playback_key_pair" => {
                self.plan_playback_key_pair(current_state, desired_input)
                    .await
            }
            "recording_configuration" => {
                self.plan_recording_configuration(current_state, desired_input)
                    .await
            }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            ...
            "stream_key" => self.create_stream_key(input).await,
            "playback_restriction_policy" => self.create_playback_restriction_policy(input).await,
            "stream" => self.create_stream(input).await,
            "playback_key_pair" => self.create_playback_key_pair(input).await,
            "recording_configuration" => self.create_recording_configuration(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "ivs", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            ...
            "stream_key" => self.read_stream_key(id).await,
            "playback_restriction_policy" => self.read_playback_restriction_policy(id).await,
            "stream" => self.read_stream(id).await,
            "playback_key_pair" => self.read_playback_key_pair(id).await,
            "recording_configuration" => self.read_recording_configuration(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "ivs", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            ...
            "stream_key" => self.update_stream_key(id, input).await,
            "playback_restriction_policy" => {
                self.update_playback_restriction_policy(id, input).await
            }
            "stream" => self.update_stream(id, input).await,
            "playback_key_pair" => self.update_playback_key_pair(id, input).await,
            "recording_configuration" => self.update_recording_configuration(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "ivs", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            ...
            "stream_key" => self.delete_stream_key(id).await,
            "playback_restriction_policy" => self.delete_playback_restriction_policy(id).await,
            "stream" => self.delete_stream(id).await,
            "playback_key_pair" => self.delete_playback_key_pair(id).await,
            "recording_configuration" => self.delete_recording_configuration(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "ivs", resource_name
            ))),
    ...
    async fn create_channel(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            let multitrack_input_configuration =
                input.get_optional_string("multitrack_input_configuration")?;
            ...
            let playback_restriction_policy_arn =
                input.get_optional_string("playback_restriction_policy_arn")?;
            let recording_configuration_arn =
                input.get_optional_string("recording_configuration_arn")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    multitrack_input_configuration.unwrap_or_default(),
                )
                ...
                .with_field(
                    ...
                    playback_restriction_policy_arn.unwrap_or_default(),
                )
                .with_field(
                    "recording_configuration_arn",
                    recording_configuration_arn.unwrap_or_default(),
                )
    ...
    async fn create_stream_key(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let channel_arn = input.get_string("channel_arn")?;
    ...
    async fn read_stream_key(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.ivs_client
    ...
    async fn update_stream_key(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let channel_arn = input.get_string("channel_arn")?;
            let tags = input.get_optional_string("tags")?;
    ...
    async fn delete_stream_key(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.ivs_client
    ...
    async fn create_playback_restriction_policy(
        ...
        self.provider.runtime().block_on(async {
            ...
            let enable_strict_origin_enforcement =
                input.get_optional_string("enable_strict_origin_enforcement")?;
            let name = input.get_optional_string("name")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    enable_strict_origin_enforcement.unwrap_or_default(),
                )
                .with_field("name", name.unwrap_or_default()))
        })
    }
    ...
    async fn read_playback_restriction_policy(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.ivs_client
    ...
    async fn delete_playback_restriction_policy(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.ivs_client
    ...
    async fn create_playback_key_pair(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_playback_key_pair(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.ivs_client
    ...
    async fn delete_playback_key_pair(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.ivs_client
    ...
    async fn create_recording_configuration(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let thumbnail_configuration = input.get_optional_string("thumbnail_configuration")?;
            let recording_reconnect_window_seconds =
                input.get_optional_string("recording_reconnect_window_seconds")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    thumbnail_configuration.unwrap_or_default(),
                )
                .with_field(
                    "recording_reconnect_window_seconds",
                    recording_reconnect_window_seconds.unwrap_or_default(),
                )
                .with_field(
                ...
                .with_field(
                    ...
                    rendition_configuration.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default()))
        })
    }
    ...
    async fn read_recording_configuration(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.ivs_client
    ...
    async fn delete_recording_configuration(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.ivs_client
