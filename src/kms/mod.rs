...
impl<'a> KmsService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            ...
            "primary_region" => self.plan_primary_region(current_state, desired_input).await,
            "key_policy" => self.plan_key_policy(current_state, desired_input).await,
            "key_rotation_status" => {
                self.plan_key_rotation_status(current_state, desired_input)
                    .await
            }
            "public_key" => self.plan_public_key(current_state, desired_input).await,
            "key_description" => {
                self.plan_key_description(current_state, desired_input)
                    .await
            }
            "alias" => self.plan_alias(current_state, desired_input).await,
            "imported_key_material" => {
                self.plan_imported_key_material(current_state, desired_input)
                    .await
            }
            "custom_key_stores" => {
                self.plan_custom_key_stores(current_state, desired_input)
                    .await
            }
            "custom_key_store" => {
                self.plan_custom_key_store(current_state, desired_input)
                    .await
            }
            "key" => self.plan_key(current_state, desired_input).await,
            "parameters_for_import" => {
                self.plan_parameters_for_import(current_state, desired_input)
                    .await
            }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            ...
            "primary_region" => self.create_primary_region(input).await,
            "key_policy" => self.create_key_policy(input).await,
            "key_rotation_status" => self.create_key_rotation_status(input).await,
            "public_key" => self.create_public_key(input).await,
            "key_description" => self.create_key_description(input).await,
            "alias" => self.create_alias(input).await,
            "imported_key_material" => self.create_imported_key_material(input).await,
            "custom_key_stores" => self.create_custom_key_stores(input).await,
            "custom_key_store" => self.create_custom_key_store(input).await,
            "key" => self.create_key(input).await,
            "parameters_for_import" => self.create_parameters_for_import(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kms", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            ...
            "primary_region" => self.read_primary_region(id).await,
            "key_policy" => self.read_key_policy(id).await,
            "key_rotation_status" => self.read_key_rotation_status(id).await,
            "public_key" => self.read_public_key(id).await,
            "key_description" => self.read_key_description(id).await,
            "alias" => self.read_alias(id).await,
            "imported_key_material" => self.read_imported_key_material(id).await,
            "custom_key_stores" => self.read_custom_key_stores(id).await,
            "custom_key_store" => self.read_custom_key_store(id).await,
            "key" => self.read_key(id).await,
            "parameters_for_import" => self.read_parameters_for_import(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kms", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            ...
            "primary_region" => self.update_primary_region(id, input).await,
            "key_policy" => self.update_key_policy(id, input).await,
            "key_rotation_status" => self.update_key_rotation_status(id, input).await,
            "public_key" => self.update_public_key(id, input).await,
            "key_description" => self.update_key_description(id, input).await,
            "alias" => self.update_alias(id, input).await,
            "imported_key_material" => self.update_imported_key_material(id, input).await,
            "custom_key_stores" => self.update_custom_key_stores(id, input).await,
            "custom_key_store" => self.update_custom_key_store(id, input).await,
            "key" => self.update_key(id, input).await,
            "parameters_for_import" => self.update_parameters_for_import(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kms", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            ...
            "primary_region" => self.delete_primary_region(id).await,
            "key_policy" => self.delete_key_policy(id).await,
            "key_rotation_status" => self.delete_key_rotation_status(id).await,
            "public_key" => self.delete_public_key(id).await,
            "key_description" => self.delete_key_description(id).await,
            "alias" => self.delete_alias(id).await,
            "imported_key_material" => self.delete_imported_key_material(id).await,
            "custom_key_stores" => self.delete_custom_key_stores(id).await,
            "custom_key_store" => self.delete_custom_key_store(id).await,
            "key" => self.delete_key(id).await,
            "parameters_for_import" => self.delete_parameters_for_import(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "kms", resource_name
            ))),
    ...
    async fn create_primary_region(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let key_id = input.get_string("key_id")?;
    ...
    async fn read_primary_region(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kms_client
    ...
    async fn delete_primary_region(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kms_client
    ...
    async fn create_key_policy(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_key_policy(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kms_client
    ...
    async fn update_key_policy(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let policy_name = input.get_optional_string("policy_name")?;
            let policy = input.get_string("policy")?;
    ...
    async fn delete_key_policy(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kms_client
    ...
    async fn create_key_rotation_status(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_key_rotation_status(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kms_client
    ...
    async fn delete_key_rotation_status(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kms_client
    ...
    async fn create_key_description(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let key_id = input.get_string("key_id")?;
    ...
    async fn read_key_description(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kms_client
    ...
    async fn delete_key_description(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kms_client
    ...
    async fn create_imported_key_material(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_imported_key_material(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kms_client
    ...
    async fn delete_imported_key_material(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kms_client
    ...
    async fn create_custom_key_stores(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_custom_key_stores(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kms_client
    ...
    async fn delete_custom_key_stores(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kms_client
    ...
    async fn create_custom_key_store(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            let xks_proxy_vpc_endpoint_service_owner =
                input.get_optional_string("xks_proxy_vpc_endpoint_service_owner")?;
            let xks_proxy_connectivity = input.get_optional_string("xks_proxy_connectivity")?;
            let xks_proxy_authentication_credential =
                input.get_optional_string("xks_proxy_authentication_credential")?;
            ...
            let xks_proxy_vpc_endpoint_service_name =
                input.get_optional_string("xks_proxy_vpc_endpoint_service_name")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    cloud_hsm_cluster_id.unwrap_or_default(),
                )
                .with_field("key_store_password", key_store_password.unwrap_or_default())
                .with_field(
                    "custom_key_store_type",
                    custom_key_store_type.unwrap_or_default(),
                )
                .with_field(
                    "xks_proxy_vpc_endpoint_service_owner",
                    xks_proxy_vpc_endpoint_service_owner.unwrap_or_default(),
                )
                .with_field(
                    "xks_proxy_connectivity",
                    xks_proxy_connectivity.unwrap_or_default(),
                )
                .with_field(
                    "xks_proxy_authentication_credential",
                    xks_proxy_authentication_credential.unwrap_or_default(),
                )
                .with_field(
                    "trust_anchor_certificate",
                    trust_anchor_certificate.unwrap_or_default(),
                )
                .with_field(
                    "xks_proxy_uri_endpoint",
                    xks_proxy_uri_endpoint.unwrap_or_default(),
                )
                .with_field(
                    "xks_proxy_vpc_endpoint_service_name",
                    xks_proxy_vpc_endpoint_service_name.unwrap_or_default(),
                )
                .with_field(
                    "custom_key_store_name",
                    custom_key_store_name.unwrap_or_default(),
                )
    ...
    async fn read_custom_key_store(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kms_client
    ...
    async fn delete_custom_key_store(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kms_client
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    customer_master_key_spec.unwrap_or_default(),
                )
                .with_field("description", description.unwrap_or_default())
                .with_field("key_spec", key_spec.unwrap_or_default())
                .with_field(
                    "custom_key_store_id",
                    custom_key_store_id.unwrap_or_default(),
                )
    ...
    async fn create_parameters_for_import(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_parameters_for_import(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.kms_client
    ...
    async fn delete_parameters_for_import(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.kms_client
