...
impl<'a> MedialiveService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "cloud_watch_alarm_template" => {
                self.plan_cloud_watch_alarm_template(current_state, desired_input)
                    .await
            }
            "multiplex" => self.plan_multiplex(current_state, desired_input).await,
            "input_device" => self.plan_input_device(current_state, desired_input).await,
            "network" => self.plan_network(current_state, desired_input).await,
            "thumbnails" => self.plan_thumbnails(current_state, desired_input).await,
            ...
            "input_device_thumbnail" => {
                self.plan_input_device_thumbnail(current_state, desired_input)
                    .await
            }
            "cluster" => self.plan_cluster(current_state, desired_input).await,
            "offering" => self.plan_offering(current_state, desired_input).await,
            "account_configuration" => {
                self.plan_account_configuration(current_state, desired_input)
                    .await
            }
            "partner_input" => self.plan_partner_input(current_state, desired_input).await,
            "input_security_group" => {
                self.plan_input_security_group(current_state, desired_input)
                    .await
            }
            "node" => self.plan_node(current_state, desired_input).await,
            "signal_map" => self.plan_signal_map(current_state, desired_input).await,
            "multiplex_program" => {
                self.plan_multiplex_program(current_state, desired_input)
                    .await
            }
            "cloud_watch_alarm_template_group" => {
                self.plan_cloud_watch_alarm_template_group(current_state, desired_input)
                    .await
            }
            "node_state" => self.plan_node_state(current_state, desired_input).await,
            "channel_class" => self.plan_channel_class(current_state, desired_input).await,
            "event_bridge_rule_template_group" => {
                self.plan_event_bridge_rule_template_group(current_state, desired_input)
                    .await
            }
            "channel_placement_group" => {
                self.plan_channel_placement_group(current_state, desired_input)
                    .await
            }
            "schedule" => self.plan_schedule(current_state, desired_input).await,
            "sdi_source" => self.plan_sdi_source(current_state, desired_input).await,
            "event_bridge_rule_template" => {
                self.plan_event_bridge_rule_template(current_state, desired_input)
                    .await
            }
            "tags" => self.plan_tags(current_state, desired_input).await,
            "node_registration_script" => {
                self.plan_node_registration_script(current_state, desired_input)
                    .await
            }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "cloud_watch_alarm_template" => self.create_cloud_watch_alarm_template(input).await,
            "multiplex" => self.create_multiplex(input).await,
            "input_device" => self.create_input_device(input).await,
            "network" => self.create_network(input).await,
            "thumbnails" => self.create_thumbnails(input).await,
            ...
            "input_device_thumbnail" => self.create_input_device_thumbnail(input).await,
            "cluster" => self.create_cluster(input).await,
            "offering" => self.create_offering(input).await,
            "account_configuration" => self.create_account_configuration(input).await,
            "partner_input" => self.create_partner_input(input).await,
            "input_security_group" => self.create_input_security_group(input).await,
            "node" => self.create_node(input).await,
            "signal_map" => self.create_signal_map(input).await,
            "multiplex_program" => self.create_multiplex_program(input).await,
            "cloud_watch_alarm_template_group" => {
                self.create_cloud_watch_alarm_template_group(input).await
            }
            "node_state" => self.create_node_state(input).await,
            "channel_class" => self.create_channel_class(input).await,
            "event_bridge_rule_template_group" => {
                self.create_event_bridge_rule_template_group(input).await
            }
            "channel_placement_group" => self.create_channel_placement_group(input).await,
            "schedule" => self.create_schedule(input).await,
            "sdi_source" => self.create_sdi_source(input).await,
            "event_bridge_rule_template" => self.create_event_bridge_rule_template(input).await,
            "tags" => self.create_tags(input).await,
            "node_registration_script" => self.create_node_registration_script(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "medialive", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "cloud_watch_alarm_template" => self.read_cloud_watch_alarm_template(id).await,
            "multiplex" => self.read_multiplex(id).await,
            "input_device" => self.read_input_device(id).await,
            "network" => self.read_network(id).await,
            "thumbnails" => self.read_thumbnails(id).await,
            "reservation" => self.read_reservation(id).await,
            "channel" => self.read_channel(id).await,
            "input" => self.read_input(id).await,
            "input_device_thumbnail" => self.read_input_device_thumbnail(id).await,
            "cluster" => self.read_cluster(id).await,
            "offering" => self.read_offering(id).await,
            "account_configuration" => self.read_account_configuration(id).await,
            "partner_input" => self.read_partner_input(id).await,
            "input_security_group" => self.read_input_security_group(id).await,
            "node" => self.read_node(id).await,
            "signal_map" => self.read_signal_map(id).await,
            "multiplex_program" => self.read_multiplex_program(id).await,
            "cloud_watch_alarm_template_group" => {
                self.read_cloud_watch_alarm_template_group(id).await
            }
            "node_state" => self.read_node_state(id).await,
            "channel_class" => self.read_channel_class(id).await,
            "event_bridge_rule_template_group" => {
                self.read_event_bridge_rule_template_group(id).await
            }
            "channel_placement_group" => self.read_channel_placement_group(id).await,
            "schedule" => self.read_schedule(id).await,
            "sdi_source" => self.read_sdi_source(id).await,
            "event_bridge_rule_template" => self.read_event_bridge_rule_template(id).await,
            "tags" => self.read_tags(id).await,
            "node_registration_script" => self.read_node_registration_script(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "medialive", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "cloud_watch_alarm_template" => self.update_cloud_watch_alarm_template(id, input).await,
            "multiplex" => self.update_multiplex(id, input).await,
            "input_device" => self.update_input_device(id, input).await,
            "network" => self.update_network(id, input).await,
            "thumbnails" => self.update_thumbnails(id, input).await,
            ...
            "input_device_thumbnail" => self.update_input_device_thumbnail(id, input).await,
            "cluster" => self.update_cluster(id, input).await,
            "offering" => self.update_offering(id, input).await,
            "account_configuration" => self.update_account_configuration(id, input).await,
            "partner_input" => self.update_partner_input(id, input).await,
            "input_security_group" => self.update_input_security_group(id, input).await,
            "node" => self.update_node(id, input).await,
            "signal_map" => self.update_signal_map(id, input).await,
            "multiplex_program" => self.update_multiplex_program(id, input).await,
            "cloud_watch_alarm_template_group" => {
                self.update_cloud_watch_alarm_template_group(id, input)
                    .await
            }
            "node_state" => self.update_node_state(id, input).await,
            "channel_class" => self.update_channel_class(id, input).await,
            "event_bridge_rule_template_group" => {
                self.update_event_bridge_rule_template_group(id, input)
                    .await
            }
            "channel_placement_group" => self.update_channel_placement_group(id, input).await,
            "schedule" => self.update_schedule(id, input).await,
            "sdi_source" => self.update_sdi_source(id, input).await,
            "event_bridge_rule_template" => self.update_event_bridge_rule_template(id, input).await,
            "tags" => self.update_tags(id, input).await,
            "node_registration_script" => self.update_node_registration_script(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "medialive", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "cloud_watch_alarm_template" => self.delete_cloud_watch_alarm_template(id).await,
            "multiplex" => self.delete_multiplex(id).await,
            "input_device" => self.delete_input_device(id).await,
            "network" => self.delete_network(id).await,
            "thumbnails" => self.delete_thumbnails(id).await,
            "reservation" => self.delete_reservation(id).await,
            "channel" => self.delete_channel(id).await,
            "input" => self.delete_input(id).await,
            "input_device_thumbnail" => self.delete_input_device_thumbnail(id).await,
            "cluster" => self.delete_cluster(id).await,
            "offering" => self.delete_offering(id).await,
            "account_configuration" => self.delete_account_configuration(id).await,
            "partner_input" => self.delete_partner_input(id).await,
            "input_security_group" => self.delete_input_security_group(id).await,
            "node" => self.delete_node(id).await,
            "signal_map" => self.delete_signal_map(id).await,
            "multiplex_program" => self.delete_multiplex_program(id).await,
            "cloud_watch_alarm_template_group" => {
                self.delete_cloud_watch_alarm_template_group(id).await
            }
            "node_state" => self.delete_node_state(id).await,
            "channel_class" => self.delete_channel_class(id).await,
            "event_bridge_rule_template_group" => {
                self.delete_event_bridge_rule_template_group(id).await
            }
            "channel_placement_group" => self.delete_channel_placement_group(id).await,
            "schedule" => self.delete_schedule(id).await,
            "sdi_source" => self.delete_sdi_source(id).await,
            "event_bridge_rule_template" => self.delete_event_bridge_rule_template(id).await,
            "tags" => self.delete_tags(id).await,
            "node_registration_script" => self.delete_node_registration_script(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "medialive", resource_name
            ))),
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    target_resource_type.unwrap_or_default(),
                )
    ...
    async fn read_cloud_watch_alarm_template(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_cloud_watch_alarm_template(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_multiplex(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_multiplex(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn update_multiplex(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let multiplex_settings = input.get_string("multiplex_settings")?;
            let availability_zones = input.get_string("availability_zones")?;
    ...
    async fn delete_multiplex(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn create_input_device(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "uhd_device_settings",
                    uhd_device_settings.unwrap_or_default(),
                )
    ...
    async fn read_input_device(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn update_input_device(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let input_device_id = input.get_string("input_device_id")?;
            let uhd_device_settings = input.get_optional_string("uhd_device_settings")?;
    ...
    async fn delete_input_device(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn create_thumbnails(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_thumbnails(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn update_thumbnails(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_thumbnails(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    cdi_input_specification.unwrap_or_default(),
                )
                ...
                .with_field(
                    "input_specification",
                    input_specification.unwrap_or_default(),
                )
                .with_field("role_arn", role_arn.unwrap_or_default())
                .with_field("tags", tags.unwrap_or_default())
                .with_field(
                    "channel_engine_version",
                    channel_engine_version.unwrap_or_default(),
                ))
        })
    }
    ...
    async fn create_input(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            let smpte2110_receiver_group_settings =
                input.get_optional_string("smpte2110_receiver_group_settings")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "media_connect_flows",
                    media_connect_flows.unwrap_or_default(),
                )
                ...
                .with_field(
                    ...
                    input_network_location.unwrap_or_default(),
                )
                .with_field(
                    "smpte2110_receiver_group_settings",
                    smpte2110_receiver_group_settings.unwrap_or_default(),
                )
                ...
                .with_field(
                    ...
                    input_security_groups.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default()))
        })
    }
    ...
    async fn create_input_device_thumbnail(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_input_device_thumbnail(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn delete_input_device_thumbnail(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn create_offering(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_offering(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn update_offering(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_offering(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn create_account_configuration(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new().with_id("placeholder-id").with_field(
                "account_configuration",
                account_configuration.unwrap_or_default(),
            ))
        })
    }
    ...
    async fn create_partner_input(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let request_id = input.get_optional_string("request_id")?;
    ...
    async fn read_partner_input(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn update_partner_input(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let request_id = input.get_optional_string("request_id")?;
            let tags = input.get_optional_string("tags")?;
    ...
    async fn delete_partner_input(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn create_input_security_group(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_input_security_group(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn delete_input_security_group(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    node_interface_mappings.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default())
    ...
    async fn create_signal_map(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let cloud_watch_alarm_template_group_identifiers =
                input.get_optional_string("cloud_watch_alarm_template_group_identifiers")?;
            let discovery_entry_point_arn = input.get_string("discovery_entry_point_arn")?;
            let event_bridge_rule_template_group_identifiers =
                input.get_optional_string("event_bridge_rule_template_group_identifiers")?;
            let name = input.get_string("name")?;
            let description = input.get_optional_string("description")?;
            let tags = input.get_optional_string("tags")?;
            let request_id = input.get_optional_string("request_id")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    cloud_watch_alarm_template_group_identifiers.unwrap_or_default(),
                )
                .with_field(
                    "discovery_entry_point_arn",
                    discovery_entry_point_arn.unwrap_or_default(),
                )
                .with_field(
                    "event_bridge_rule_template_group_identifiers",
                    event_bridge_rule_template_group_identifiers.unwrap_or_default(),
                )
                .with_field("name", name.unwrap_or_default())
                .with_field("description", description.unwrap_or_default())
                .with_field("tags", tags.unwrap_or_default())
    ...
    async fn read_signal_map(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn update_signal_map(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let cloud_watch_alarm_template_group_identifiers =
                input.get_optional_string("cloud_watch_alarm_template_group_identifiers")?;
    ...
    async fn delete_signal_map(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn create_multiplex_program(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let request_id = input.get_string("request_id")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    multiplex_program_settings.unwrap_or_default(),
                )
    ...
    async fn read_multiplex_program(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn delete_multiplex_program(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn read_cloud_watch_alarm_template_group(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn delete_cloud_watch_alarm_template_group(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn create_node_state(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let cluster_id = input.get_string("cluster_id")?;
    ...
    async fn read_node_state(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn update_node_state(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let cluster_id = input.get_string("cluster_id")?;
            let node_id = input.get_string("node_id")?;
    ...
    async fn delete_node_state(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn create_channel_class(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_channel_class(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn update_channel_class(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let channel_class = input.get_string("channel_class")?;
            let channel_id = input.get_string("channel_id")?;
    ...
    async fn delete_channel_class(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn read_event_bridge_rule_template_group(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn delete_event_bridge_rule_template_group(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn create_channel_placement_group(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let cluster_id = input.get_string("cluster_id")?;
    ...
    async fn read_channel_placement_group(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn delete_channel_placement_group(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn create_sdi_source(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let r#type = input.get_optional_string("type")?;
    ...
    async fn read_sdi_source(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn update_sdi_source(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let r#type = input.get_optional_string("type")?;
            let name = input.get_optional_string("name")?;
    ...
    async fn delete_sdi_source(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn read_event_bridge_rule_template(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn delete_event_bridge_rule_template(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
    ...
    async fn read_node_registration_script(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.medialive_client
    ...
    async fn delete_node_registration_script(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.medialive_client
