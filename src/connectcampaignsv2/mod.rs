...
impl<'a> Connectcampaignsv2Service<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "instance_onboarding_job" => {
                self.plan_instance_onboarding_job(current_state, desired_input)
                    .await
            }
            "outbound_request_batch" => {
                self.plan_outbound_request_batch(current_state, desired_input)
                    .await
            }
            "campaign_channel_subtype_config" => {
                self.plan_campaign_channel_subtype_config(current_state, desired_input)
                    .await
            }
            "instance_communication_limits" => {
                self.plan_instance_communication_limits(current_state, desired_input)
                    .await
            }
            "campaign_communication_limits" => {
                self.plan_campaign_communication_limits(current_state, desired_input)
                    .await
            }
            "connect_instance_integration" => {
                self.plan_connect_instance_integration(current_state, desired_input)
                    .await
            }
            "profile_outbound_request_batch" => {
                self.plan_profile_outbound_request_batch(current_state, desired_input)
                    .await
            }
            "campaign_state" => self.plan_campaign_state(current_state, desired_input).await,
            "campaign_flow_association" => {
                self.plan_campaign_flow_association(current_state, desired_input)
                    .await
            }
            "campaign" => self.plan_campaign(current_state, desired_input).await,
            "campaign_name" => self.plan_campaign_name(current_state, desired_input).await,
            "campaign_source" => {
                self.plan_campaign_source(current_state, desired_input)
                    .await
            }
            "campaign_schedule" => {
                self.plan_campaign_schedule(current_state, desired_input)
                    .await
            }
            "connect_instance_config" => {
                self.plan_connect_instance_config(current_state, desired_input)
                    .await
            }
            "instance_onboarding_job_status" => {
                self.plan_instance_onboarding_job_status(current_state, desired_input)
                    .await
            }
            "campaign_communication_time" => {
                self.plan_campaign_communication_time(current_state, desired_input)
                    .await
            }
            "campaign_state_batch" => {
                self.plan_campaign_state_batch(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "connectcampaignsv2", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "instance_onboarding_job" => self.create_instance_onboarding_job(input).await,
            "outbound_request_batch" => self.create_outbound_request_batch(input).await,
            "campaign_channel_subtype_config" => {
            ...
            "connect_instance_integration" => self.create_connect_instance_integration(input).await,
            "profile_outbound_request_batch" => {
                self.create_profile_outbound_request_batch(input).await
            }
            "campaign_state" => self.create_campaign_state(input).await,
            "campaign_flow_association" => self.create_campaign_flow_association(input).await,
            "campaign" => self.create_campaign(input).await,
            "campaign_name" => self.create_campaign_name(input).await,
            "campaign_source" => self.create_campaign_source(input).await,
            "campaign_schedule" => self.create_campaign_schedule(input).await,
            "connect_instance_config" => self.create_connect_instance_config(input).await,
            "instance_onboarding_job_status" => {
                self.create_instance_onboarding_job_status(input).await
            }
            "campaign_communication_time" => self.create_campaign_communication_time(input).await,
            "campaign_state_batch" => self.create_campaign_state_batch(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "connectcampaignsv2", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "instance_onboarding_job" => self.read_instance_onboarding_job(id).await,
            "outbound_request_batch" => self.read_outbound_request_batch(id).await,
            "campaign_channel_subtype_config" => {
                self.read_campaign_channel_subtype_config(id).await
            }
            "instance_communication_limits" => self.read_instance_communication_limits(id).await,
            "campaign_communication_limits" => self.read_campaign_communication_limits(id).await,
            "connect_instance_integration" => self.read_connect_instance_integration(id).await,
            "profile_outbound_request_batch" => self.read_profile_outbound_request_batch(id).await,
            "campaign_state" => self.read_campaign_state(id).await,
            "campaign_flow_association" => self.read_campaign_flow_association(id).await,
            "campaign" => self.read_campaign(id).await,
            "campaign_name" => self.read_campaign_name(id).await,
            "campaign_source" => self.read_campaign_source(id).await,
            "campaign_schedule" => self.read_campaign_schedule(id).await,
            "connect_instance_config" => self.read_connect_instance_config(id).await,
            "instance_onboarding_job_status" => self.read_instance_onboarding_job_status(id).await,
            "campaign_communication_time" => self.read_campaign_communication_time(id).await,
            "campaign_state_batch" => self.read_campaign_state_batch(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "connectcampaignsv2", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "instance_onboarding_job" => self.update_instance_onboarding_job(id, input).await,
            "outbound_request_batch" => self.update_outbound_request_batch(id, input).await,
            "campaign_channel_subtype_config" => {
            ...
            "campaign_state" => self.update_campaign_state(id, input).await,
            "campaign_flow_association" => self.update_campaign_flow_association(id, input).await,
            "campaign" => self.update_campaign(id, input).await,
            "campaign_name" => self.update_campaign_name(id, input).await,
            "campaign_source" => self.update_campaign_source(id, input).await,
            "campaign_schedule" => self.update_campaign_schedule(id, input).await,
            "connect_instance_config" => self.update_connect_instance_config(id, input).await,
            "instance_onboarding_job_status" => {
            ...
            "campaign_state_batch" => self.update_campaign_state_batch(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "connectcampaignsv2", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "instance_onboarding_job" => self.delete_instance_onboarding_job(id).await,
            "outbound_request_batch" => self.delete_outbound_request_batch(id).await,
            "campaign_channel_subtype_config" => {
                self.delete_campaign_channel_subtype_config(id).await
            }
            "instance_communication_limits" => self.delete_instance_communication_limits(id).await,
            "campaign_communication_limits" => self.delete_campaign_communication_limits(id).await,
            "connect_instance_integration" => self.delete_connect_instance_integration(id).await,
            "profile_outbound_request_batch" => {
                self.delete_profile_outbound_request_batch(id).await
            }
            "campaign_state" => self.delete_campaign_state(id).await,
            "campaign_flow_association" => self.delete_campaign_flow_association(id).await,
            "campaign" => self.delete_campaign(id).await,
            "campaign_name" => self.delete_campaign_name(id).await,
            "campaign_source" => self.delete_campaign_source(id).await,
            "campaign_schedule" => self.delete_campaign_schedule(id).await,
            "connect_instance_config" => self.delete_connect_instance_config(id).await,
            "instance_onboarding_job_status" => {
                self.delete_instance_onboarding_job_status(id).await
            }
            "campaign_communication_time" => self.delete_campaign_communication_time(id).await,
            "campaign_state_batch" => self.delete_campaign_state_batch(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "connectcampaignsv2", resource_name
            ))),
    ...
    async fn create_instance_onboarding_job(&self, input: ResourceInput) -> Result<ResourceOutput> {
    ...
    async fn read_instance_onboarding_job(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_instance_onboarding_job(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_outbound_request_batch(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_outbound_request_batch(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_outbound_request_batch(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    channel_subtype_config.unwrap_or_default(),
                ))
        })
    }

    /// Read a campaign_channel_subtype_config resource
    async fn read_campaign_channel_subtype_config(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_campaign_channel_subtype_config(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
    ...
    async fn create_instance_communication_limits(
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "connect_instance_id",
                    connect_instance_id.unwrap_or_default(),
                )
                .with_field(
                    "communication_limits_config",
                    communication_limits_config.unwrap_or_default(),
                ))
        })
    }

    /// Read a instance_communication_limits resource
    async fn read_instance_communication_limits(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_instance_communication_limits(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
    ...
    async fn create_campaign_communication_limits(
        ...
        self.provider.runtime().block_on(async {
            // Extract input fields
            let communication_limits_override =
                input.get_string("communication_limits_override")?;
            let id = input.get_string("id")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    communication_limits_override.unwrap_or_default(),
                )
                .with_field("id", id.unwrap_or_default()))
        })
    }
    ...
    async fn read_campaign_communication_limits(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_campaign_communication_limits(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
    ...
    async fn read_connect_instance_integration(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_connect_instance_integration(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    profile_outbound_requests.unwrap_or_default(),
                ))
        })
    }

    /// Read a profile_outbound_request_batch resource
    async fn read_profile_outbound_request_batch(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_profile_outbound_request_batch(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
    ...
    async fn create_campaign_state(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_campaign_state(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_campaign_state(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    connect_campaign_flow_arn.unwrap_or_default(),
                )
                .with_field("id", id.unwrap_or_default()))
        })
    }
    ...
    async fn read_campaign_flow_association(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_campaign_flow_association(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
    ...
    async fn create_campaign(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            let connect_instance_id = input.get_string("connect_instance_id")?;
            let communication_time_config =
                input.get_optional_string("communication_time_config")?;
            let name = input.get_string("name")?;
            let tags = input.get_optional_string("tags")?;
            let communication_limits_override =
                input.get_optional_string("communication_limits_override")?;
            let schedule = input.get_optional_string("schedule")?;
            let connect_campaign_flow_arn =
                input.get_optional_string("connect_campaign_flow_arn")?;
            let channel_subtype_config = input.get_string("channel_subtype_config")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    communication_time_config.unwrap_or_default(),
                )
    ...
    async fn create_campaign_name(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let id = input.get_string("id")?;
    ...
    async fn read_campaign_name(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn update_campaign_name(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let id = input.get_string("id")?;
            let name = input.get_string("name")?;
    ...
    async fn delete_campaign_name(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
    ...
    async fn create_campaign_source(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_campaign_source(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_campaign_source(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
    ...
    async fn create_campaign_schedule(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let id = input.get_string("id")?;
    ...
    async fn read_campaign_schedule(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_campaign_schedule(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
    ...
    async fn create_connect_instance_config(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_connect_instance_config(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_connect_instance_config(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
    ...
    async fn read_instance_onboarding_job_status(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_instance_onboarding_job_status(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
    ...
    async fn read_campaign_communication_time(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_campaign_communication_time(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
    ...
    async fn create_campaign_state_batch(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_campaign_state_batch(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.connectcampaignsv2_client
    ...
    async fn delete_campaign_state_batch(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.connectcampaignsv2_client
