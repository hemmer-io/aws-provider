...
impl<'a> QappsService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "q_app" => {
                self.plan_q_app(current_state, desired_input).await
            }
            "library_item" => {
                self.plan_library_item(current_state, desired_input).await
            }
            "presigned_url" => {
                self.plan_presigned_url(current_state, desired_input).await
            }
            "q_app_session" => {
                self.plan_q_app_session(current_state, desired_input).await
            }
            "q_app_permissions" => {
                self.plan_q_app_permissions(current_state, desired_input).await
            }
            "q_app_session_metadata" => {
                self.plan_q_app_session_metadata(current_state, desired_input).await
            }
            "library_item_metadata" => {
                self.plan_library_item_metadata(current_state, desired_input).await
            }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "q_app" => {
                self.create_q_app(input).await
            }
            "library_item" => {
                self.create_library_item(input).await
            }
            "presigned_url" => {
                self.create_presigned_url(input).await
            }
            "q_app_session" => {
                self.create_q_app_session(input).await
            }
            "q_app_permissions" => {
                self.create_q_app_permissions(input).await
            }
            "q_app_session_metadata" => {
                self.create_q_app_session_metadata(input).await
            }
            "library_item_metadata" => {
                self.create_library_item_metadata(input).await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "qapps",
    ...
    pub async fn read_resource(
        ...
        match resource_name {
            "q_app" => {
                self.read_q_app(id).await
            }
            "library_item" => {
                self.read_library_item(id).await
            }
            "presigned_url" => {
                self.read_presigned_url(id).await
            }
            "q_app_session" => {
                self.read_q_app_session(id).await
            }
            "q_app_permissions" => {
                self.read_q_app_permissions(id).await
            }
            "q_app_session_metadata" => {
                self.read_q_app_session_metadata(id).await
            }
            "library_item_metadata" => {
                self.read_library_item_metadata(id).await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "qapps",
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "q_app" => {
                self.update_q_app(id, input).await
            }
            "library_item" => {
                self.update_library_item(id, input).await
            }
            "presigned_url" => {
                self.update_presigned_url(id, input).await
            }
            "q_app_session" => {
                self.update_q_app_session(id, input).await
            }
            "q_app_permissions" => {
                self.update_q_app_permissions(id, input).await
            }
            "q_app_session_metadata" => {
                self.update_q_app_session_metadata(id, input).await
            }
            "library_item_metadata" => {
                self.update_library_item_metadata(id, input).await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "qapps",
    ...
    pub async fn delete_resource(
        ...
        match resource_name {
            "q_app" => {
                self.delete_q_app(id).await
            }
            "library_item" => {
                self.delete_library_item(id).await
            }
            "presigned_url" => {
                self.delete_presigned_url(id).await
            }
            "q_app_session" => {
                self.delete_q_app_session(id).await
            }
            "q_app_permissions" => {
                self.delete_q_app_permissions(id).await
            }
            "q_app_session_metadata" => {
                self.delete_q_app_session_metadata(id).await
            }
            "library_item_metadata" => {
                self.delete_library_item_metadata(id).await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "qapps",
    ...
    async fn create_presigned_url(
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field("app_id", app_id.unwrap_or_default())
                .with_field("file_contents_sha256", file_contents_sha256.unwrap_or_default())
                .with_field("instance_id", instance_id.unwrap_or_default())
    ...
    async fn create_q_app_permissions(
        &self,
        input: ResourceInput,
    ) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
    ...
    async fn read_q_app_permissions(
        &self,
        id: &str,
    ) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_q_app_permissions(
        &self,
        id: &str,
    ) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_q_app_session_metadata(
        &self,
        input: ResourceInput,
    ) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field("sharing_configuration", sharing_configuration.unwrap_or_default())
            )
        })
    }

    /// Read a q_app_session_metadata resource
    async fn read_q_app_session_metadata(
        &self,
        id: &str,
    ) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_q_app_session_metadata(
        &self,
        id: &str,
    ) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_library_item_metadata(
        &self,
        input: ResourceInput,
    ) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
    ...
    async fn read_library_item_metadata(
        &self,
        id: &str,
    ) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_library_item_metadata(
        &self,
        id: &str,
    ) -> Result<()> {
        self.provider.runtime().block_on(async {
