...
impl<'a> TrustedadvisorService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "organization_recommendation_lifecycle" => {
                self.plan_organization_recommendation_lifecycle(current_state, desired_input)
                    .await
            }
            "recommendation_lifecycle" => {
                self.plan_recommendation_lifecycle(current_state, desired_input)
                    .await
            }
            "recommendation" => self.plan_recommendation(current_state, desired_input).await,
            "organization_recommendation" => {
                self.plan_organization_recommendation(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "trustedadvisor", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "organization_recommendation_lifecycle" => {
                self.create_organization_recommendation_lifecycle(input)
                    .await
            }
            "recommendation_lifecycle" => self.create_recommendation_lifecycle(input).await,
            "recommendation" => self.create_recommendation(input).await,
            "organization_recommendation" => self.create_organization_recommendation(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "trustedadvisor", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            ...
            "recommendation_lifecycle" => self.read_recommendation_lifecycle(id).await,
            "recommendation" => self.read_recommendation(id).await,
            "organization_recommendation" => self.read_organization_recommendation(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "trustedadvisor", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "organization_recommendation_lifecycle" => {
                self.update_organization_recommendation_lifecycle(id, input)
                    .await
            }
            "recommendation_lifecycle" => self.update_recommendation_lifecycle(id, input).await,
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            ...
            "recommendation_lifecycle" => self.delete_recommendation_lifecycle(id).await,
            "recommendation" => self.delete_recommendation(id).await,
            "organization_recommendation" => self.delete_organization_recommendation(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "trustedadvisor", resource_name
            ))),
    ...
    async fn create_organization_recommendation_lifecycle(
        ...
        self.provider.runtime().block_on(async {
            ...
            let organization_recommendation_identifier =
                input.get_string("organization_recommendation_identifier")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    organization_recommendation_identifier.unwrap_or_default(),
                )
    ...
    async fn read_organization_recommendation_lifecycle(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_organization_recommendation_lifecycle(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    recommendation_identifier.unwrap_or_default(),
                ))
        })
    }

    /// Read a recommendation_lifecycle resource
    async fn read_recommendation_lifecycle(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.trustedadvisor_client
    ...
    async fn delete_recommendation_lifecycle(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.trustedadvisor_client
    ...
    async fn read_organization_recommendation(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.trustedadvisor_client
    ...
    async fn delete_organization_recommendation(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.trustedadvisor_client
