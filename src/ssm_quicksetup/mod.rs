...
impl<'a> Ssm_quicksetupService<'a> {
    ...
    async fn create_service_settings(
        ...
        self.provider.runtime().block_on(async {
            // Extract input fields
            let explorer_enabling_role_arn = input.get_optional_string("explorer_enabling_role_arn")?;
            ...
            Ok(ResourceOutput::new()
                .with_id("placeholder-id")
                .with_field("explorer_enabling_role_arn", explorer_enabling_role_arn.unwrap_or_default())
            )
        })
    }
