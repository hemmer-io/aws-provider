...
impl<'a> MwaaService<'a> {
    ...
    async fn create_web_login_token(
        &self,
        input: ResourceInput,
    ) -> Result<ResourceOutput> {
    ...
    async fn delete_web_login_token(
        &self,
        id: &str,
    ) -> Result<()> {
        self.provider.runtime().block_on(async {
