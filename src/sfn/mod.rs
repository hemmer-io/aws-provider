...
impl<'a> SfnService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "state_machine" => self.plan_state_machine(current_state, desired_input).await,
            "execution_history" => {
                self.plan_execution_history(current_state, desired_input)
                    .await
            }
            "state_machine_for_execution" => {
                self.plan_state_machine_for_execution(current_state, desired_input)
                    .await
            }
            "activity" => self.plan_activity(current_state, desired_input).await,
            "state_machine_alias" => {
                self.plan_state_machine_alias(current_state, desired_input)
                    .await
            }
            "activity_task" => self.plan_activity_task(current_state, desired_input).await,
            "execution" => self.plan_execution(current_state, desired_input).await,
            "map_run" => self.plan_map_run(current_state, desired_input).await,
            "state_machine_version" => {
                self.plan_state_machine_version(current_state, desired_input)
                    .await
            }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "state_machine" => self.create_state_machine(input).await,
            "execution_history" => self.create_execution_history(input).await,
            "state_machine_for_execution" => self.create_state_machine_for_execution(input).await,
            "activity" => self.create_activity(input).await,
            "state_machine_alias" => self.create_state_machine_alias(input).await,
            "activity_task" => self.create_activity_task(input).await,
            "execution" => self.create_execution(input).await,
            "map_run" => self.create_map_run(input).await,
            "state_machine_version" => self.create_state_machine_version(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "sfn", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "state_machine" => self.read_state_machine(id).await,
            "execution_history" => self.read_execution_history(id).await,
            "state_machine_for_execution" => self.read_state_machine_for_execution(id).await,
            "activity" => self.read_activity(id).await,
            "state_machine_alias" => self.read_state_machine_alias(id).await,
            "activity_task" => self.read_activity_task(id).await,
            "execution" => self.read_execution(id).await,
            "map_run" => self.read_map_run(id).await,
            "state_machine_version" => self.read_state_machine_version(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "sfn", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "state_machine" => self.update_state_machine(id, input).await,
            "execution_history" => self.update_execution_history(id, input).await,
            "state_machine_for_execution" => {
                self.update_state_machine_for_execution(id, input).await
            }
            "activity" => self.update_activity(id, input).await,
            "state_machine_alias" => self.update_state_machine_alias(id, input).await,
            "activity_task" => self.update_activity_task(id, input).await,
            "execution" => self.update_execution(id, input).await,
            "map_run" => self.update_map_run(id, input).await,
            "state_machine_version" => self.update_state_machine_version(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "sfn", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "state_machine" => self.delete_state_machine(id).await,
            "execution_history" => self.delete_execution_history(id).await,
            "state_machine_for_execution" => self.delete_state_machine_for_execution(id).await,
            "activity" => self.delete_activity(id).await,
            "state_machine_alias" => self.delete_state_machine_alias(id).await,
            "activity_task" => self.delete_activity_task(id).await,
            "execution" => self.delete_execution(id).await,
            "map_run" => self.delete_map_run(id).await,
            "state_machine_version" => self.delete_state_machine_version(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "sfn", resource_name
            ))),
    ...
    async fn create_state_machine(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    tracing_configuration.unwrap_or_default(),
                )
                .with_field(
    ...
    async fn read_state_machine(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn update_state_machine(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let definition = input.get_string("definition")?;
            let logging_configuration = input.get_optional_string("logging_configuration")?;
    ...
    async fn delete_state_machine(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_execution_history(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_execution_history(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.sfn_client
    ...
    async fn delete_execution_history(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.sfn_client
    ...
    async fn read_state_machine_for_execution(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.sfn_client
    ...
    async fn delete_state_machine_for_execution(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.sfn_client
    ...
    async fn create_activity(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let encryption_configuration = input.get_optional_string("encryption_configuration")?;
    ...
    async fn read_activity(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.sfn_client
    ...
    async fn update_activity(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let encryption_configuration = input.get_optional_string("encryption_configuration")?;
            let name = input.get_string("name")?;
    ...
    async fn create_state_machine_alias(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    routing_configuration.unwrap_or_default(),
                )
    ...
    async fn read_state_machine_alias(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.sfn_client
    ...
    async fn delete_state_machine_alias(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.sfn_client
    ...
    async fn create_activity_task(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_activity_task(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.sfn_client
    ...
    async fn update_activity_task(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_activity_task(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.sfn_client
    ...
    async fn create_map_run(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            let tolerated_failure_percentage =
                input.get_optional_string("tolerated_failure_percentage")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    tolerated_failure_count.unwrap_or_default(),
                )
                .with_field("map_run_arn", map_run_arn.unwrap_or_default())
                .with_field("max_concurrency", max_concurrency.unwrap_or_default())
                .with_field(
                    "tolerated_failure_percentage",
                    tolerated_failure_percentage.unwrap_or_default(),
                ))
        })
    }

    /// Read a map_run resource
    async fn read_map_run(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.sfn_client
    ...
    async fn update_map_run(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let tolerated_failure_count = input.get_optional_string("tolerated_failure_count")?;
            let map_run_arn = input.get_string("map_run_arn")?;
    ...
    async fn delete_map_run(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.sfn_client
    ...
    async fn create_state_machine_version(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_state_machine_version(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.sfn_client
    ...
    async fn delete_state_machine_version(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.sfn_client
