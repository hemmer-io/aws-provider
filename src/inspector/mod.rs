...
impl<'a> InspectorService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            ...
            "resource_group" => self.plan_resource_group(current_state, desired_input).await,
            "findings" => self.plan_findings(current_state, desired_input).await,
            "rules_packages" => self.plan_rules_packages(current_state, desired_input).await,
            "cross_account_access_role" => {
                self.plan_cross_account_access_role(current_state, desired_input)
                    .await
            }
            "exclusions" => self.plan_exclusions(current_state, desired_input).await,
            "assessment_targets" => {
                self.plan_assessment_targets(current_state, desired_input)
                    .await
            }
            "assessment_template" => {
                self.plan_assessment_template(current_state, desired_input)
                    .await
            }
            "resource_groups" => {
                self.plan_resource_groups(current_state, desired_input)
                    .await
            }
            "telemetry_metadata" => {
                self.plan_telemetry_metadata(current_state, desired_input)
                    .await
            }
            "assessment_target" => {
                self.plan_assessment_target(current_state, desired_input)
                    .await
            }
            "exclusions_preview" => {
                self.plan_exclusions_preview(current_state, desired_input)
                    .await
            }
            "assessment_runs" => {
                self.plan_assessment_runs(current_state, desired_input)
                    .await
            }
            "assessment_templates" => {
                self.plan_assessment_templates(current_state, desired_input)
                    .await
            }
            "assessment_run" => self.plan_assessment_run(current_state, desired_input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "inspector", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            ...
            "resource_group" => self.create_resource_group(input).await,
            "findings" => self.create_findings(input).await,
            "rules_packages" => self.create_rules_packages(input).await,
            "cross_account_access_role" => self.create_cross_account_access_role(input).await,
            "exclusions" => self.create_exclusions(input).await,
            "assessment_targets" => self.create_assessment_targets(input).await,
            "assessment_template" => self.create_assessment_template(input).await,
            "resource_groups" => self.create_resource_groups(input).await,
            "telemetry_metadata" => self.create_telemetry_metadata(input).await,
            "assessment_target" => self.create_assessment_target(input).await,
            "exclusions_preview" => self.create_exclusions_preview(input).await,
            "assessment_runs" => self.create_assessment_runs(input).await,
            "assessment_templates" => self.create_assessment_templates(input).await,
            "assessment_run" => self.create_assessment_run(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "inspector", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            ...
            "resource_group" => self.read_resource_group(id).await,
            "findings" => self.read_findings(id).await,
            "rules_packages" => self.read_rules_packages(id).await,
            "cross_account_access_role" => self.read_cross_account_access_role(id).await,
            "exclusions" => self.read_exclusions(id).await,
            "assessment_targets" => self.read_assessment_targets(id).await,
            "assessment_template" => self.read_assessment_template(id).await,
            "resource_groups" => self.read_resource_groups(id).await,
            "telemetry_metadata" => self.read_telemetry_metadata(id).await,
            "assessment_target" => self.read_assessment_target(id).await,
            "exclusions_preview" => self.read_exclusions_preview(id).await,
            "assessment_runs" => self.read_assessment_runs(id).await,
            "assessment_templates" => self.read_assessment_templates(id).await,
            "assessment_run" => self.read_assessment_run(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "inspector", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            ...
            "resource_group" => self.update_resource_group(id, input).await,
            "findings" => self.update_findings(id, input).await,
            "rules_packages" => self.update_rules_packages(id, input).await,
            "cross_account_access_role" => self.update_cross_account_access_role(id, input).await,
            "exclusions" => self.update_exclusions(id, input).await,
            "assessment_targets" => self.update_assessment_targets(id, input).await,
            "assessment_template" => self.update_assessment_template(id, input).await,
            "resource_groups" => self.update_resource_groups(id, input).await,
            "telemetry_metadata" => self.update_telemetry_metadata(id, input).await,
            "assessment_target" => self.update_assessment_target(id, input).await,
            "exclusions_preview" => self.update_exclusions_preview(id, input).await,
            "assessment_runs" => self.update_assessment_runs(id, input).await,
            "assessment_templates" => self.update_assessment_templates(id, input).await,
            "assessment_run" => self.update_assessment_run(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "inspector", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            ...
            "resource_group" => self.delete_resource_group(id).await,
            "findings" => self.delete_findings(id).await,
            "rules_packages" => self.delete_rules_packages(id).await,
            "cross_account_access_role" => self.delete_cross_account_access_role(id).await,
            "exclusions" => self.delete_exclusions(id).await,
            "assessment_targets" => self.delete_assessment_targets(id).await,
            "assessment_template" => self.delete_assessment_template(id).await,
            "resource_groups" => self.delete_resource_groups(id).await,
            "telemetry_metadata" => self.delete_telemetry_metadata(id).await,
            "assessment_target" => self.delete_assessment_target(id).await,
            "exclusions_preview" => self.delete_exclusions_preview(id).await,
            "assessment_runs" => self.delete_assessment_runs(id).await,
            "assessment_templates" => self.delete_assessment_templates(id).await,
            "assessment_run" => self.delete_assessment_run(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "inspector", resource_name
            ))),
    ...
    async fn create_resource_group(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new().with_id("placeholder-id").with_field(
                "resource_group_tags",
                resource_group_tags.unwrap_or_default(),
            ))
        })
    }

    /// Read a resource_group resource
    async fn read_resource_group(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn delete_resource_group(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
    ...
    async fn create_rules_packages(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_rules_packages(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn delete_rules_packages(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
    ...
    async fn read_cross_account_access_role(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn delete_cross_account_access_role(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
    ...
    async fn create_exclusions(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_exclusions(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn update_exclusions(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_exclusions(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
    ...
    async fn create_assessment_targets(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_assessment_targets(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn delete_assessment_targets(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
    ...
    async fn create_assessment_template(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let duration_in_seconds = input.get_string("duration_in_seconds")?;
            let rules_package_arns = input.get_string("rules_package_arns")?;
            let user_attributes_for_findings =
                input.get_optional_string("user_attributes_for_findings")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "duration_in_seconds",
                    duration_in_seconds.unwrap_or_default(),
                )
                .with_field("rules_package_arns", rules_package_arns.unwrap_or_default())
                .with_field(
                    "user_attributes_for_findings",
                    user_attributes_for_findings.unwrap_or_default(),
                )
                .with_field(
                    "assessment_target_arn",
                    assessment_target_arn.unwrap_or_default(),
                )
                .with_field(
                    "assessment_template_name",
                    assessment_template_name.unwrap_or_default(),
                ))
        })
    }

    /// Read a assessment_template resource
    async fn read_assessment_template(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn delete_assessment_template(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
    ...
    async fn create_resource_groups(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_resource_groups(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn delete_resource_groups(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
    ...
    async fn create_telemetry_metadata(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_telemetry_metadata(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn delete_telemetry_metadata(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
    ...
    async fn create_assessment_target(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    assessment_target_name.unwrap_or_default(),
                )
    ...
    async fn read_assessment_target(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn delete_assessment_target(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
    ...
    async fn create_exclusions_preview(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new().with_id("placeholder-id").with_field(
                ...
                assessment_template_arn.unwrap_or_default(),
            ))
        })
    }

    /// Read a exclusions_preview resource
    async fn read_exclusions_preview(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn delete_exclusions_preview(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
    ...
    async fn create_assessment_runs(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_assessment_runs(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn delete_assessment_runs(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
    ...
    async fn create_assessment_templates(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_assessment_templates(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn delete_assessment_templates(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
    ...
    async fn create_assessment_run(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_assessment_run(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.inspector_client
    ...
    async fn delete_assessment_run(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.inspector_client
