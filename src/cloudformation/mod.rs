...
impl<'a> CloudformationService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "stack_refactor" => self.plan_stack_refactor(current_state, desired_input).await,
            "account_limits" => self.plan_account_limits(current_state, desired_input).await,
            "stack_resource_drifts" => {
                self.plan_stack_resource_drifts(current_state, desired_input)
                    .await
            }
            "termination_protection" => {
                self.plan_termination_protection(current_state, desired_input)
                    .await
            }
            "stack" => self.plan_stack(current_state, desired_input).await,
            "publisher" => self.plan_publisher(current_state, desired_input).await,
            "type_" => self.plan_type_(current_state, desired_input).await,
            "stack_instance" => self.plan_stack_instance(current_state, desired_input).await,
            "type_registration" => {
                self.plan_type_registration(current_state, desired_input)
                    .await
            }
            "stack_events" => self.plan_stack_events(current_state, desired_input).await,
            "template" => self.plan_template(current_state, desired_input).await,
            "change_set" => self.plan_change_set(current_state, desired_input).await,
            "organizations_access" => {
                self.plan_organizations_access(current_state, desired_input)
                    .await
            }
            "stack_set_operation" => {
                self.plan_stack_set_operation(current_state, desired_input)
                    .await
            }
            "generated_template" => {
                self.plan_generated_template(current_state, desired_input)
                    .await
            }
            "stacks" => self.plan_stacks(current_state, desired_input).await,
            "stack_set" => self.plan_stack_set(current_state, desired_input).await,
            "resource_scan" => self.plan_resource_scan(current_state, desired_input).await,
            "stack_instances" => {
                self.plan_stack_instances(current_state, desired_input)
                    .await
            }
            "stack_resources" => {
                self.plan_stack_resources(current_state, desired_input)
                    .await
            }
            "template_summary" => {
                self.plan_template_summary(current_state, desired_input)
                    .await
            }
            "change_set_hooks" => {
                self.plan_change_set_hooks(current_state, desired_input)
                    .await
            }
            "stack_drift_detection_status" => {
                self.plan_stack_drift_detection_status(current_state, desired_input)
                    .await
            }
            "stack_resource" => self.plan_stack_resource(current_state, desired_input).await,
            "stack_policy" => self.plan_stack_policy(current_state, desired_input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "cloudformation", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "stack_refactor" => self.create_stack_refactor(input).await,
            "account_limits" => self.create_account_limits(input).await,
            "stack_resource_drifts" => self.create_stack_resource_drifts(input).await,
            "termination_protection" => self.create_termination_protection(input).await,
            "stack" => self.create_stack(input).await,
            "publisher" => self.create_publisher(input).await,
            "type_" => self.create_type_(input).await,
            "stack_instance" => self.create_stack_instance(input).await,
            "type_registration" => self.create_type_registration(input).await,
            "stack_events" => self.create_stack_events(input).await,
            "template" => self.create_template(input).await,
            "change_set" => self.create_change_set(input).await,
            "organizations_access" => self.create_organizations_access(input).await,
            "stack_set_operation" => self.create_stack_set_operation(input).await,
            "generated_template" => self.create_generated_template(input).await,
            "stacks" => self.create_stacks(input).await,
            "stack_set" => self.create_stack_set(input).await,
            "resource_scan" => self.create_resource_scan(input).await,
            "stack_instances" => self.create_stack_instances(input).await,
            "stack_resources" => self.create_stack_resources(input).await,
            "template_summary" => self.create_template_summary(input).await,
            "change_set_hooks" => self.create_change_set_hooks(input).await,
            "stack_drift_detection_status" => self.create_stack_drift_detection_status(input).await,
            "stack_resource" => self.create_stack_resource(input).await,
            "stack_policy" => self.create_stack_policy(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "cloudformation", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "stack_refactor" => self.read_stack_refactor(id).await,
            "account_limits" => self.read_account_limits(id).await,
            "stack_resource_drifts" => self.read_stack_resource_drifts(id).await,
            "termination_protection" => self.read_termination_protection(id).await,
            "stack" => self.read_stack(id).await,
            "publisher" => self.read_publisher(id).await,
            "type_" => self.read_type_(id).await,
            "stack_instance" => self.read_stack_instance(id).await,
            "type_registration" => self.read_type_registration(id).await,
            "stack_events" => self.read_stack_events(id).await,
            "template" => self.read_template(id).await,
            "change_set" => self.read_change_set(id).await,
            "organizations_access" => self.read_organizations_access(id).await,
            "stack_set_operation" => self.read_stack_set_operation(id).await,
            "generated_template" => self.read_generated_template(id).await,
            "stacks" => self.read_stacks(id).await,
            "stack_set" => self.read_stack_set(id).await,
            "resource_scan" => self.read_resource_scan(id).await,
            "stack_instances" => self.read_stack_instances(id).await,
            "stack_resources" => self.read_stack_resources(id).await,
            "template_summary" => self.read_template_summary(id).await,
            "change_set_hooks" => self.read_change_set_hooks(id).await,
            "stack_drift_detection_status" => self.read_stack_drift_detection_status(id).await,
            "stack_resource" => self.read_stack_resource(id).await,
            "stack_policy" => self.read_stack_policy(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "cloudformation", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "stack_refactor" => self.update_stack_refactor(id, input).await,
            "account_limits" => self.update_account_limits(id, input).await,
            "stack_resource_drifts" => self.update_stack_resource_drifts(id, input).await,
            "termination_protection" => self.update_termination_protection(id, input).await,
            "stack" => self.update_stack(id, input).await,
            "publisher" => self.update_publisher(id, input).await,
            "type_" => self.update_type_(id, input).await,
            "stack_instance" => self.update_stack_instance(id, input).await,
            "type_registration" => self.update_type_registration(id, input).await,
            "stack_events" => self.update_stack_events(id, input).await,
            "template" => self.update_template(id, input).await,
            "change_set" => self.update_change_set(id, input).await,
            "organizations_access" => self.update_organizations_access(id, input).await,
            "stack_set_operation" => self.update_stack_set_operation(id, input).await,
            "generated_template" => self.update_generated_template(id, input).await,
            "stacks" => self.update_stacks(id, input).await,
            "stack_set" => self.update_stack_set(id, input).await,
            "resource_scan" => self.update_resource_scan(id, input).await,
            "stack_instances" => self.update_stack_instances(id, input).await,
            "stack_resources" => self.update_stack_resources(id, input).await,
            "template_summary" => self.update_template_summary(id, input).await,
            "change_set_hooks" => self.update_change_set_hooks(id, input).await,
            "stack_drift_detection_status" => {
                self.update_stack_drift_detection_status(id, input).await
            }
            "stack_resource" => self.update_stack_resource(id, input).await,
            "stack_policy" => self.update_stack_policy(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "cloudformation", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "stack_refactor" => self.delete_stack_refactor(id).await,
            "account_limits" => self.delete_account_limits(id).await,
            "stack_resource_drifts" => self.delete_stack_resource_drifts(id).await,
            "termination_protection" => self.delete_termination_protection(id).await,
            "stack" => self.delete_stack(id).await,
            "publisher" => self.delete_publisher(id).await,
            "type_" => self.delete_type_(id).await,
            "stack_instance" => self.delete_stack_instance(id).await,
            "type_registration" => self.delete_type_registration(id).await,
            "stack_events" => self.delete_stack_events(id).await,
            "template" => self.delete_template(id).await,
            "change_set" => self.delete_change_set(id).await,
            "organizations_access" => self.delete_organizations_access(id).await,
            "stack_set_operation" => self.delete_stack_set_operation(id).await,
            "generated_template" => self.delete_generated_template(id).await,
            "stacks" => self.delete_stacks(id).await,
            "stack_set" => self.delete_stack_set(id).await,
            "resource_scan" => self.delete_resource_scan(id).await,
            "stack_instances" => self.delete_stack_instances(id).await,
            "stack_resources" => self.delete_stack_resources(id).await,
            "template_summary" => self.delete_template_summary(id).await,
            "change_set_hooks" => self.delete_change_set_hooks(id).await,
            "stack_drift_detection_status" => self.delete_stack_drift_detection_status(id).await,
            "stack_resource" => self.delete_stack_resource(id).await,
            "stack_policy" => self.delete_stack_policy(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "cloudformation", resource_name
            ))),
    ...
    async fn create_stack_refactor(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    enable_stack_creation.unwrap_or_default(),
                )
    ...
    async fn read_stack_refactor(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_stack_refactor(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_stack_resource_drifts(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_stack_resource_drifts(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_stack_resource_drifts(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_termination_protection(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let enable_termination_protection =
                input.get_string("enable_termination_protection")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    enable_termination_protection.unwrap_or_default(),
                )
    ...
    async fn read_termination_protection(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_termination_protection(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_stack(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            let enable_termination_protection =
                input.get_optional_string("enable_termination_protection")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    retain_except_on_create.unwrap_or_default(),
                )
                .with_field(
    ...
    async fn create_publisher(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_publisher(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn update_publisher(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_publisher(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_stack_instance(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_stack_instance(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_stack_instance(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_type_registration(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_type_registration(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_type_registration(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_stack_events(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_stack_events(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn update_stack_events(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_stack_events(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_change_set(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let template_url = input.get_optional_string("template_url")?;
            ...
            let import_existing_resources =
                input.get_optional_string("import_existing_resources")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    use_previous_template.unwrap_or_default(),
                )
                .with_field(
                    "rollback_configuration",
                    rollback_configuration.unwrap_or_default(),
                ...
                .with_field(
                    ...
                    include_nested_stacks.unwrap_or_default(),
                )
                .with_field(
                    "resources_to_import",
                    resources_to_import.unwrap_or_default(),
                )
                .with_field(
                    "import_existing_resources",
                    import_existing_resources.unwrap_or_default(),
                )
    ...
    async fn read_change_set(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn update_change_set(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let template_url = input.get_optional_string("template_url")?;
            let parameters = input.get_optional_string("parameters")?;
    ...
    async fn delete_change_set(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_organizations_access(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_organizations_access(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_organizations_access(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_stack_set_operation(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_stack_set_operation(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_stack_set_operation(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_generated_template(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    generated_template_name.unwrap_or_default(),
                )
                .with_field("stack_name", stack_name.unwrap_or_default())
                .with_field(
                    "template_configuration",
                    template_configuration.unwrap_or_default(),
                ))
        })
    }

    /// Read a generated_template resource
    async fn read_generated_template(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_generated_template(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_stack_set(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    administration_role_arn.unwrap_or_default(),
                )
                ...
                .with_field(
                    "execution_role_name",
                    execution_role_name.unwrap_or_default(),
                )
                .with_field("template_body", template_body.unwrap_or_default())
    ...
    async fn read_stack_set(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn update_stack_set(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let administration_role_arn = input.get_optional_string("administration_role_arn")?;
            let call_as = input.get_optional_string("call_as")?;
    ...
    async fn delete_stack_set(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_resource_scan(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_resource_scan(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn update_resource_scan(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_resource_scan(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_stack_instances(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    operation_preferences.unwrap_or_default(),
                )
                .with_field(
                    "parameter_overrides",
    ...
    async fn read_stack_instances(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_stack_instances(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_stack_resources(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_stack_resources(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_stack_resources(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_template_summary(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_template_summary(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_template_summary(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_change_set_hooks(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_change_set_hooks(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_change_set_hooks(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn read_stack_drift_detection_status(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_stack_drift_detection_status(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_stack_resource(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_stack_resource(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn delete_stack_resource(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
    ...
    async fn create_stack_policy(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_stack_policy(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.cloudformation_client
    ...
    async fn update_stack_policy(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_stack_policy(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.cloudformation_client
