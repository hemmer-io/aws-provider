...
impl<'a> Customer_profilesService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "similar_profiles" => {
                self.plan_similar_profiles(current_state, desired_input)
                    .await
            }
            "segment_snapshot" => {
                self.plan_segment_snapshot(current_state, desired_input)
                    .await
            }
            "integration" => self.plan_integration(current_state, desired_input).await,
            "profile_object_type" => {
                self.plan_profile_object_type(current_state, desired_input)
                    .await
            }
            "matches" => self.plan_matches(current_state, desired_input).await,
            "profile_object" => self.plan_profile_object(current_state, desired_input).await,
            ...
            "workflow_steps" => self.plan_workflow_steps(current_state, desired_input).await,
            "event_trigger" => self.plan_event_trigger(current_state, desired_input).await,
            "profile" => self.plan_profile(current_state, desired_input).await,
            "profile_key" => self.plan_profile_key(current_state, desired_input).await,
            "calculated_attribute_definition" => {
                self.plan_calculated_attribute_definition(current_state, desired_input)
                    .await
            }
            "domain_layout" => self.plan_domain_layout(current_state, desired_input).await,
            "auto_merging_preview" => {
                self.plan_auto_merging_preview(current_state, desired_input)
                    .await
            }
            "calculated_attribute_for_profile" => {
                self.plan_calculated_attribute_for_profile(current_state, desired_input)
                    .await
            }
            "upload_job" => self.plan_upload_job(current_state, desired_input).await,
            "integration_workflow" => {
                self.plan_integration_workflow(current_state, desired_input)
                    .await
            }
            "domain" => self.plan_domain(current_state, desired_input).await,
            "segment_estimate" => {
                self.plan_segment_estimate(current_state, desired_input)
                    .await
            }
            "profile_history_record" => {
                self.plan_profile_history_record(current_state, desired_input)
                    .await
            }
            "profile_object_type_template" => {
                self.plan_profile_object_type_template(current_state, desired_input)
                    .await
            }
            "segment_definition" => {
                self.plan_segment_definition(current_state, desired_input)
                    .await
            }
            "identity_resolution_job" => {
                self.plan_identity_resolution_job(current_state, desired_input)
                    .await
            }
            "upload_job_path" => {
                self.plan_upload_job_path(current_state, desired_input)
                    .await
            }
            "segment_membership" => {
                self.plan_segment_membership(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "customer_profiles", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "similar_profiles" => self.create_similar_profiles(input).await,
            "segment_snapshot" => self.create_segment_snapshot(input).await,
            "integration" => self.create_integration(input).await,
            "profile_object_type" => self.create_profile_object_type(input).await,
            "matches" => self.create_matches(input).await,
            "profile_object" => self.create_profile_object(input).await,
            "workflow" => self.create_workflow(input).await,
            "event_stream" => self.create_event_stream(input).await,
            "workflow_steps" => self.create_workflow_steps(input).await,
            "event_trigger" => self.create_event_trigger(input).await,
            "profile" => self.create_profile(input).await,
            "profile_key" => self.create_profile_key(input).await,
            "calculated_attribute_definition" => {
                self.create_calculated_attribute_definition(input).await
            }
            "domain_layout" => self.create_domain_layout(input).await,
            "auto_merging_preview" => self.create_auto_merging_preview(input).await,
            "calculated_attribute_for_profile" => {
                self.create_calculated_attribute_for_profile(input).await
            }
            "upload_job" => self.create_upload_job(input).await,
            "integration_workflow" => self.create_integration_workflow(input).await,
            "domain" => self.create_domain(input).await,
            "segment_estimate" => self.create_segment_estimate(input).await,
            "profile_history_record" => self.create_profile_history_record(input).await,
            "profile_object_type_template" => self.create_profile_object_type_template(input).await,
            "segment_definition" => self.create_segment_definition(input).await,
            "identity_resolution_job" => self.create_identity_resolution_job(input).await,
            "upload_job_path" => self.create_upload_job_path(input).await,
            "segment_membership" => self.create_segment_membership(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "customer_profiles", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "similar_profiles" => self.read_similar_profiles(id).await,
            "segment_snapshot" => self.read_segment_snapshot(id).await,
            "integration" => self.read_integration(id).await,
            "profile_object_type" => self.read_profile_object_type(id).await,
            "matches" => self.read_matches(id).await,
            "profile_object" => self.read_profile_object(id).await,
            "workflow" => self.read_workflow(id).await,
            "event_stream" => self.read_event_stream(id).await,
            "workflow_steps" => self.read_workflow_steps(id).await,
            "event_trigger" => self.read_event_trigger(id).await,
            "profile" => self.read_profile(id).await,
            "profile_key" => self.read_profile_key(id).await,
            "calculated_attribute_definition" => {
                self.read_calculated_attribute_definition(id).await
            }
            "domain_layout" => self.read_domain_layout(id).await,
            "auto_merging_preview" => self.read_auto_merging_preview(id).await,
            "calculated_attribute_for_profile" => {
                self.read_calculated_attribute_for_profile(id).await
            }
            "upload_job" => self.read_upload_job(id).await,
            "integration_workflow" => self.read_integration_workflow(id).await,
            "domain" => self.read_domain(id).await,
            "segment_estimate" => self.read_segment_estimate(id).await,
            "profile_history_record" => self.read_profile_history_record(id).await,
            "profile_object_type_template" => self.read_profile_object_type_template(id).await,
            "segment_definition" => self.read_segment_definition(id).await,
            "identity_resolution_job" => self.read_identity_resolution_job(id).await,
            "upload_job_path" => self.read_upload_job_path(id).await,
            "segment_membership" => self.read_segment_membership(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "customer_profiles", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "similar_profiles" => self.update_similar_profiles(id, input).await,
            "segment_snapshot" => self.update_segment_snapshot(id, input).await,
            "integration" => self.update_integration(id, input).await,
            "profile_object_type" => self.update_profile_object_type(id, input).await,
            "matches" => self.update_matches(id, input).await,
            "profile_object" => self.update_profile_object(id, input).await,
            "workflow" => self.update_workflow(id, input).await,
            "event_stream" => self.update_event_stream(id, input).await,
            "workflow_steps" => self.update_workflow_steps(id, input).await,
            "event_trigger" => self.update_event_trigger(id, input).await,
            "profile" => self.update_profile(id, input).await,
            "profile_key" => self.update_profile_key(id, input).await,
            "calculated_attribute_definition" => {
                self.update_calculated_attribute_definition(id, input).await
            }
            "domain_layout" => self.update_domain_layout(id, input).await,
            "auto_merging_preview" => self.update_auto_merging_preview(id, input).await,
            "calculated_attribute_for_profile" => {
                self.update_calculated_attribute_for_profile(id, input)
                    .await
            }
            "upload_job" => self.update_upload_job(id, input).await,
            "integration_workflow" => self.update_integration_workflow(id, input).await,
            "domain" => self.update_domain(id, input).await,
            "segment_estimate" => self.update_segment_estimate(id, input).await,
            "profile_history_record" => self.update_profile_history_record(id, input).await,
            "profile_object_type_template" => {
                self.update_profile_object_type_template(id, input).await
            }
            "segment_definition" => self.update_segment_definition(id, input).await,
            "identity_resolution_job" => self.update_identity_resolution_job(id, input).await,
            "upload_job_path" => self.update_upload_job_path(id, input).await,
            "segment_membership" => self.update_segment_membership(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "customer_profiles", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "similar_profiles" => self.delete_similar_profiles(id).await,
            "segment_snapshot" => self.delete_segment_snapshot(id).await,
            "integration" => self.delete_integration(id).await,
            "profile_object_type" => self.delete_profile_object_type(id).await,
            "matches" => self.delete_matches(id).await,
            "profile_object" => self.delete_profile_object(id).await,
            "workflow" => self.delete_workflow(id).await,
            "event_stream" => self.delete_event_stream(id).await,
            "workflow_steps" => self.delete_workflow_steps(id).await,
            "event_trigger" => self.delete_event_trigger(id).await,
            "profile" => self.delete_profile(id).await,
            "profile_key" => self.delete_profile_key(id).await,
            "calculated_attribute_definition" => {
                self.delete_calculated_attribute_definition(id).await
            }
            "domain_layout" => self.delete_domain_layout(id).await,
            "auto_merging_preview" => self.delete_auto_merging_preview(id).await,
            "calculated_attribute_for_profile" => {
                self.delete_calculated_attribute_for_profile(id).await
            }
            "upload_job" => self.delete_upload_job(id).await,
            "integration_workflow" => self.delete_integration_workflow(id).await,
            "domain" => self.delete_domain(id).await,
            "segment_estimate" => self.delete_segment_estimate(id).await,
            "profile_history_record" => self.delete_profile_history_record(id).await,
            "profile_object_type_template" => self.delete_profile_object_type_template(id).await,
            "segment_definition" => self.delete_segment_definition(id).await,
            "identity_resolution_job" => self.delete_identity_resolution_job(id).await,
            "upload_job_path" => self.delete_upload_job_path(id).await,
            "segment_membership" => self.delete_segment_membership(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "customer_profiles", resource_name
            ))),
    ...
    async fn create_similar_profiles(&self, input: ResourceInput) -> Result<ResourceOutput> {
    ...
    async fn read_similar_profiles(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn delete_similar_profiles(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_segment_snapshot(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    segment_definition_name.unwrap_or_default(),
                )
    ...
    async fn read_segment_snapshot(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_segment_snapshot(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_integration(&self, input: ResourceInput) -> Result<ResourceOutput> {
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "event_trigger_names",
                    event_trigger_names.unwrap_or_default(),
                )
    ...
    async fn create_profile_object_type(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let encryption_key = input.get_optional_string("encryption_key")?;
            let source_last_updated_timestamp_format =
                input.get_optional_string("source_last_updated_timestamp_format")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    source_last_updated_timestamp_format.unwrap_or_default(),
                )
                ...
                .with_field(
                    ...
                    allow_profile_creation.unwrap_or_default(),
                )
                .with_field(
                    "max_profile_object_count",
                    max_profile_object_count.unwrap_or_default(),
                )
    ...
    async fn read_profile_object_type(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_profile_object_type(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_matches(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_matches(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn update_matches(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_matches(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_profile_object(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_profile_object(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_profile_object(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_workflow_steps(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_workflow_steps(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_workflow_steps(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_event_trigger(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    event_trigger_conditions.unwrap_or_default(),
                )
                .with_field(
                    "event_trigger_limits",
                    event_trigger_limits.unwrap_or_default(),
                )
                .with_field("object_type_name", object_type_name.unwrap_or_default())
    ...
    async fn read_event_trigger(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn update_event_trigger(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let segment_filter = input.get_optional_string("segment_filter")?;
            let domain_name = input.get_string("domain_name")?;
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    business_phone_number.unwrap_or_default(),
                )
                .with_field("birth_date", birth_date.unwrap_or_default())
                .with_field("first_name", first_name.unwrap_or_default())
                .with_field(
                    "mobile_phone_number",
                    mobile_phone_number.unwrap_or_default(),
                )
                .with_field(
                    "personal_email_address",
                    personal_email_address.unwrap_or_default(),
                )
                .with_field(
                    "business_email_address",
                    business_email_address.unwrap_or_default(),
                )
    ...
    async fn create_profile_key(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_profile_key(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn update_profile_key(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_profile_key(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_calculated_attribute_definition(
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    "use_historical_data",
                    use_historical_data.unwrap_or_default(),
                )
                ...
                .with_field(
                    ...
                    calculated_attribute_name.unwrap_or_default(),
                )
    ...
    async fn read_calculated_attribute_definition(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_calculated_attribute_definition(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_domain_layout(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    layout_definition_name.unwrap_or_default(),
                )
                .with_field("description", description.unwrap_or_default())
    ...
    async fn read_domain_layout(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn update_domain_layout(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let layout_definition_name = input.get_string("layout_definition_name")?;
            let description = input.get_string("description")?;
    ...
    async fn delete_domain_layout(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_auto_merging_preview(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_auto_merging_preview(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_auto_merging_preview(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn read_calculated_attribute_for_profile(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_calculated_attribute_for_profile(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_upload_job(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_upload_job(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn update_upload_job(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let fields = input.get_string("fields")?;
            let unique_key = input.get_string("unique_key")?;
    ...
    async fn delete_upload_job(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_integration_workflow(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let object_type_name = input.get_string("object_type_name")?;
    ...
    async fn read_integration_workflow(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_integration_workflow(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    dead_letter_queue_url.unwrap_or_default(),
                )
                .with_field(
                    "default_encryption_key",
                    default_encryption_key.unwrap_or_default(),
                )
                .with_field("matching", matching.unwrap_or_default())
                .with_field("domain_name", domain_name.unwrap_or_default())
                .with_field(
                    "rule_based_matching",
                    rule_based_matching.unwrap_or_default(),
                )
                .with_field(
                    "default_expiration_days",
                    default_expiration_days.unwrap_or_default(),
                )
                .with_field("tags", tags.unwrap_or_default()))
        })
    }
    ...
    async fn create_segment_estimate(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let domain_name = input.get_string("domain_name")?;
    ...
    async fn read_segment_estimate(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_segment_estimate(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_profile_history_record(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_profile_history_record(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_profile_history_record(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn read_profile_object_type_template(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_profile_object_type_template(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_segment_definition(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let display_name = input.get_string("display_name")?;
    ...
    async fn read_segment_definition(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_segment_definition(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_identity_resolution_job(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_identity_resolution_job(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_identity_resolution_job(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_upload_job_path(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_upload_job_path(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_upload_job_path(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
    ...
    async fn create_segment_membership(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_segment_membership(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.customer_profiles_client
    ...
    async fn delete_segment_membership(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.customer_profiles_client
