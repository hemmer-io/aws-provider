...
impl<'a> LakeformationService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "lf_tag" => self.plan_lf_tag(current_state, desired_input).await,
            "query_statistics" => {
                self.plan_query_statistics(current_state, desired_input)
                    .await
            }
            "table_objects" => self.plan_table_objects(current_state, desired_input).await,
            "lf_tag_expression" => {
                self.plan_lf_tag_expression(current_state, desired_input)
                    .await
            }
            "temporary_glue_partition_credentials" => {
                self.plan_temporary_glue_partition_credentials(current_state, desired_input)
                    .await
            }
            "query_state" => self.plan_query_state(current_state, desired_input).await,
            "resource" => self.plan_resource(current_state, desired_input).await,
            "effective_permissions_for_path" => {
                self.plan_effective_permissions_for_path(current_state, desired_input)
                    .await
            }
            "work_unit_results" => {
                self.plan_work_unit_results(current_state, desired_input)
                    .await
            }
            "objects_on_cancel" => {
                self.plan_objects_on_cancel(current_state, desired_input)
                    .await
            }
            "work_units" => self.plan_work_units(current_state, desired_input).await,
            "data_lake_settings" => {
                self.plan_data_lake_settings(current_state, desired_input)
                    .await
            }
            "transaction" => self.plan_transaction(current_state, desired_input).await,
            "lake_formation_identity_center_configuration" => {
                self.plan_lake_formation_identity_center_configuration(current_state, desired_input)
                    .await
            }
            "resource_lf_tags" => {
                self.plan_resource_lf_tags(current_state, desired_input)
                    .await
            }
            "data_cells_filter" => {
                self.plan_data_cells_filter(current_state, desired_input)
                    .await
            }
            "table_storage_optimizer" => {
                self.plan_table_storage_optimizer(current_state, desired_input)
                    .await
            }
            "lake_formation_opt_in" => {
                self.plan_lake_formation_opt_in(current_state, desired_input)
                    .await
            }
            "data_lake_principal" => {
                self.plan_data_lake_principal(current_state, desired_input)
                    .await
            }
            "temporary_glue_table_credentials" => {
                self.plan_temporary_glue_table_credentials(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "lakeformation", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "lf_tag" => self.create_lf_tag(input).await,
            "query_statistics" => self.create_query_statistics(input).await,
            "table_objects" => self.create_table_objects(input).await,
            "lf_tag_expression" => self.create_lf_tag_expression(input).await,
            "temporary_glue_partition_credentials" => {
                self.create_temporary_glue_partition_credentials(input)
                    .await
            }
            "query_state" => self.create_query_state(input).await,
            "resource" => self.create_resource(input).await,
            "effective_permissions_for_path" => {
                self.create_effective_permissions_for_path(input).await
            }
            "work_unit_results" => self.create_work_unit_results(input).await,
            "objects_on_cancel" => self.create_objects_on_cancel(input).await,
            "work_units" => self.create_work_units(input).await,
            "data_lake_settings" => self.create_data_lake_settings(input).await,
            "transaction" => self.create_transaction(input).await,
            "lake_formation_identity_center_configuration" => {
                self.create_lake_formation_identity_center_configuration(input)
                    .await
            }
            "resource_lf_tags" => self.create_resource_lf_tags(input).await,
            "data_cells_filter" => self.create_data_cells_filter(input).await,
            "table_storage_optimizer" => self.create_table_storage_optimizer(input).await,
            "lake_formation_opt_in" => self.create_lake_formation_opt_in(input).await,
            "data_lake_principal" => self.create_data_lake_principal(input).await,
            "temporary_glue_table_credentials" => {
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "lf_tag" => self.read_lf_tag(id).await,
            "query_statistics" => self.read_query_statistics(id).await,
            "table_objects" => self.read_table_objects(id).await,
            "lf_tag_expression" => self.read_lf_tag_expression(id).await,
            "temporary_glue_partition_credentials" => {
                self.read_temporary_glue_partition_credentials(id).await
            }
            "query_state" => self.read_query_state(id).await,
            "resource" => self.read_resource(id).await,
            "effective_permissions_for_path" => self.read_effective_permissions_for_path(id).await,
            "work_unit_results" => self.read_work_unit_results(id).await,
            "objects_on_cancel" => self.read_objects_on_cancel(id).await,
            "work_units" => self.read_work_units(id).await,
            "data_lake_settings" => self.read_data_lake_settings(id).await,
            "transaction" => self.read_transaction(id).await,
            "lake_formation_identity_center_configuration" => {
                self.read_lake_formation_identity_center_configuration(id)
                    .await
            }
            "resource_lf_tags" => self.read_resource_lf_tags(id).await,
            "data_cells_filter" => self.read_data_cells_filter(id).await,
            "table_storage_optimizer" => self.read_table_storage_optimizer(id).await,
            "lake_formation_opt_in" => self.read_lake_formation_opt_in(id).await,
            "data_lake_principal" => self.read_data_lake_principal(id).await,
            "temporary_glue_table_credentials" => {
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "lf_tag" => self.update_lf_tag(id, input).await,
            "query_statistics" => self.update_query_statistics(id, input).await,
            "table_objects" => self.update_table_objects(id, input).await,
            "lf_tag_expression" => self.update_lf_tag_expression(id, input).await,
            "temporary_glue_partition_credentials" => {
                self.update_temporary_glue_partition_credentials(id, input)
                    .await
            }
            "query_state" => self.update_query_state(id, input).await,
            "resource" => self.update_resource(id, input).await,
            "effective_permissions_for_path" => {
                self.update_effective_permissions_for_path(id, input).await
            }
            "work_unit_results" => self.update_work_unit_results(id, input).await,
            "objects_on_cancel" => self.update_objects_on_cancel(id, input).await,
            "work_units" => self.update_work_units(id, input).await,
            "data_lake_settings" => self.update_data_lake_settings(id, input).await,
            "transaction" => self.update_transaction(id, input).await,
            "lake_formation_identity_center_configuration" => {
                self.update_lake_formation_identity_center_configuration(id, input)
                    .await
            }
            "resource_lf_tags" => self.update_resource_lf_tags(id, input).await,
            "data_cells_filter" => self.update_data_cells_filter(id, input).await,
            "table_storage_optimizer" => self.update_table_storage_optimizer(id, input).await,
            "lake_formation_opt_in" => self.update_lake_formation_opt_in(id, input).await,
            "data_lake_principal" => self.update_data_lake_principal(id, input).await,
            "temporary_glue_table_credentials" => {
                self.update_temporary_glue_table_credentials(id, input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "lf_tag" => self.delete_lf_tag(id).await,
            "query_statistics" => self.delete_query_statistics(id).await,
            "table_objects" => self.delete_table_objects(id).await,
            "lf_tag_expression" => self.delete_lf_tag_expression(id).await,
            "temporary_glue_partition_credentials" => {
                self.delete_temporary_glue_partition_credentials(id).await
            }
            "query_state" => self.delete_query_state(id).await,
            "resource" => self.delete_resource(id).await,
            "effective_permissions_for_path" => {
                self.delete_effective_permissions_for_path(id).await
            }
            "work_unit_results" => self.delete_work_unit_results(id).await,
            "objects_on_cancel" => self.delete_objects_on_cancel(id).await,
            "work_units" => self.delete_work_units(id).await,
            "data_lake_settings" => self.delete_data_lake_settings(id).await,
            "transaction" => self.delete_transaction(id).await,
            "lake_formation_identity_center_configuration" => {
                self.delete_lake_formation_identity_center_configuration(id)
                    .await
            }
            "resource_lf_tags" => self.delete_resource_lf_tags(id).await,
            "data_cells_filter" => self.delete_data_cells_filter(id).await,
            "table_storage_optimizer" => self.delete_table_storage_optimizer(id).await,
            "lake_formation_opt_in" => self.delete_lake_formation_opt_in(id).await,
            "data_lake_principal" => self.delete_data_lake_principal(id).await,
            "temporary_glue_table_credentials" => {
    ...
    async fn create_lf_tag(&self, input: ResourceInput) -> Result<ResourceOutput> {
    ...
    async fn read_lf_tag(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
    ...
    async fn update_lf_tag(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let catalog_id = input.get_optional_string("catalog_id")?;
            let tag_key = input.get_string("tag_key")?;
    ...
    async fn delete_lf_tag(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
    ...
    async fn create_query_statistics(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_query_statistics(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_query_statistics(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_table_objects(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_table_objects(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn update_table_objects(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
            let transaction_id = input.get_optional_string("transaction_id")?;
            let catalog_id = input.get_optional_string("catalog_id")?;
    ...
    async fn delete_table_objects(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_lf_tag_expression(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_lf_tag_expression(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_lf_tag_expression(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn read_temporary_glue_partition_credentials(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_temporary_glue_partition_credentials(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_query_state(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_query_state(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn update_query_state(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_query_state(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
        ...
        self.provider.runtime().block_on(async {
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    hybrid_access_enabled.unwrap_or_default(),
                )
    ...
    async fn read_effective_permissions_for_path(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_effective_permissions_for_path(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_work_unit_results(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_work_unit_results(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_work_unit_results(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_objects_on_cancel(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_objects_on_cancel(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_objects_on_cancel(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_work_units(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_work_units(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn update_work_units(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_work_units(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_data_lake_settings(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let catalog_id = input.get_optional_string("catalog_id")?;
    ...
    async fn read_data_lake_settings(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_data_lake_settings(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_transaction(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_transaction(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn update_transaction(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn delete_lake_formation_identity_center_configuration(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_resource_lf_tags(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_resource_lf_tags(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_resource_lf_tags(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_data_cells_filter(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_data_cells_filter(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_data_cells_filter(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_table_storage_optimizer(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
            let table_name = input.get_string("table_name")?;
            ...
            Ok(ResourceOutput::new()
                ...
                .with_field(
                    ...
                    storage_optimizer_config.unwrap_or_default(),
                )
                .with_field("catalog_id", catalog_id.unwrap_or_default()))
        })
    }
    ...
    async fn read_table_storage_optimizer(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_table_storage_optimizer(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_lake_formation_opt_in(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_lake_formation_opt_in(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_lake_formation_opt_in(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn create_data_lake_principal(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // Use the runtime to execute async SDK calls
        self.provider.runtime().block_on(async {
            // Extract input fields
    ...
    async fn read_data_lake_principal(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_data_lake_principal(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
    ...
    async fn read_temporary_glue_table_credentials(&self, id: &str) -> Result<ResourceOutput> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to read the resource
            // Example:
            // let result = self.provider.lakeformation_client
    ...
    async fn delete_temporary_glue_table_credentials(&self, id: &str) -> Result<()> {
        self.provider.runtime().block_on(async {
            // TODO: Call AWS SDK to delete the resource
            // Example:
            // self.provider.lakeformation_client
